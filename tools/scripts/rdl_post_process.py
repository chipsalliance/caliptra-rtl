...
def fix_unpacked_struct(line: str) -> str:
    ...
    The original root-cause is **Verilator**: it requires every member
    of a struct to be similarly driven, and errors on the
    force-statements Caliptra's DV applies to these signals when the
    struct is unpacked.  Packing the struct sidesteps that restriction.

    **Category:** upstream workaround.
    **Removal:** when we can adopt a newer Verilator that accepts
    force-statements against unpacked structs across the *entire*
    Caliptra test-suite (at which point packing is no longer required),
    or when peakrdl-regblock emits ``struct packed`` directly (or
    exposes a knob to do so).  As of 1.3.1 this still fires thousands of
    times per regeneration, so the fix is load-bearing.
    """

...
# Arg checks
if [[ $# -ne 2 ]]; then
    echo "Error, requires feature branch name argument and PAT for adams-bridge"
    exit 1
else
...
if [[ -z "${adamsbridge_pat:+'empty'}" ]]; then
    echo "Error, got empty PAT argument"
    exit 1
fi

if [[ -z "${ADAMSBRIDGE_ROOT:+"empty"}" ]]; then
    echo "Error, must set ADAMSBRIDGE_ROOT"
    exit 1
fi
cd $ADAMSBRIDGE_ROOT
...
# Stamp target branch
bash ${ADAMSBRIDGE_ROOT}/.github/scripts/stamp_repo.sh
merge_branch=$(git branch --show-current)
git commit -m "MICROSOFT AUTOMATED PIPELINE: Stamp '${merge_branch}' with updated timestamp and hash after successful run" -- ${ADAMSBRIDGE_ROOT}/.github/workflow_metadata/pr_hash ${ADAMSBRIDGE_ROOT}/.github/workflow_metadata/pr_timestamp
...
git restore --source=${merge_branch} ${ADAMSBRIDGE_ROOT}/.github/workflow_metadata/pr_timestamp
git restore --source=${merge_branch} ${ADAMSBRIDGE_ROOT}/.github/workflow_metadata/pr_hash
git commit -m "MICROSOFT AUTOMATED PIPELINE: Stamp '${feature_branch}' with updated timestamp and hash after successful run" -- ${ADAMSBRIDGE_ROOT}/.github/workflow_metadata/pr_hash ${ADAMSBRIDGE_ROOT}/.github/workflow_metadata/pr_timestamp
sts=$(git push $(sed "s,github.com,microsoft_pipeline:${adamsbridge_pat}@github.com," <<< ${REMOTE_ADDR}) ${feature_branch}:${feature_branch} > /dev/null; echo $?)
if [[ ${sts} -ne 0 ]]; then

...
All inputs and output locations are taken from command-line arguments;
the script reads no environment variables.

Shared type definitions (e.g. ``kv_def.rdl``) are supplied via
``--preload FILE`` (repeatable).  Each preload is ``compile_file()``d
into the same ``RDLCompiler`` ahead of the target RDL, populating the
shared root scope with the types the target references by name.  This is
the systemrdl-compiler idiom for cross-file definitions - see
https://systemrdl-compiler.readthedocs.io/en/latest/dev_notes/multi_file_compilation.html
- and it is preferred over ``\`include``-ing shared-defs files, which
re-declares the same types in the shared namespace and fails to compile.
Use ``-I/--include-dir`` for intra-file ``\`include`` fragments that make
up a single addrmap (e.g. ``soc_ifc_reg_properties.rdl``).
...
def compile_and_elaborate(
    ...
    Resolve cross-file type dependencies (e.g. the shared ``kv_def.rdl``)
    by preloading: compile each path in *preload_files* into the shared
    ``RDLCompiler`` before the target file, so the target can reference
    those types by name from the shared root scope.  See the module
    docstring for why this is preferred over ``\`include``.

    Pass *include_dirs* to the preprocessor's search path for intra-file
    ``\`include`` directives that pull in source fragments of a single
    addrmap (e.g. ``soc_ifc_reg_properties.rdl``).

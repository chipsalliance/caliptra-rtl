...
files_missing_header=$(grep -r -L -i  --exclude-dir={uvmf*,.git,cmark,caliptra_reg_html,caliptra_top_reg_html,sha256,sha512,doe,aes_secworks,fw_test_*} --exclude=*.{tcl,txt,js,htm,html,json,vf,yml,woff,rsp,rdl,bashrc,waiver,cfg,hex,rc,exe,pdf} --exclude=*_reg*.{sv,rdl} --exclude=*_csr*.* --exclude={sglint_waivers,.git-comodules,.gitignore,spyglass_lint.policy,ascent.ctl} --exclude=c_sample.c Apache)

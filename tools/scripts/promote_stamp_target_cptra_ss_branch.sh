...
# Arg checks
if [[ $# -ne 2 ]]; then
    echo "Error, requires feature branch name argument and PAT for caliptra-ss"
    exit 1
else
...
if [[ -z "${caliptra_ss_pat:+'empty'}" ]]; then
...
# Stamp target branch
bash ${CALIPTRA_SS_ROOT}/.github/scripts/stamp_repo.sh
merge_branch=$(git branch --show-current)
git commit -m "MICROSOFT AUTOMATED PIPELINE: Stamp '${merge_branch}' with updated timestamp and hash after successful run" -- ${CALIPTRA_SS_ROOT}/.github/workflow_metadata/pr_hash ${CALIPTRA_SS_ROOT}/.github/workflow_metadata/pr_timestamp
...
git restore --source=${merge_branch} ${CALIPTRA_SS_ROOT}/.github/workflow_metadata/pr_timestamp
git restore --source=${merge_branch} ${CALIPTRA_SS_ROOT}/.github/workflow_metadata/pr_hash
git commit -m "MICROSOFT AUTOMATED PIPELINE: Stamp '${feature_branch}' with updated timestamp and hash after successful run" -- ${CALIPTRA_SS_ROOT}/.github/workflow_metadata/pr_hash ${CALIPTRA_SS_ROOT}/.github/workflow_metadata/pr_timestamp
sts=$(git push $(sed "s,github.com,microsoft_pipeline:${caliptra_ss_pat}@github.com," <<< ${REMOTE_ADDR}) ${feature_branch}:${feature_branch} > /dev/null; echo $?)
if [[ ${sts} -ne 0 ]]; then

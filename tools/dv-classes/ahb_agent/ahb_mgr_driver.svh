  ...
  if (m_cur_data_phase == null) begin
    ...
    if (m_cur_data_phase.m_write) begin
      ...
      if (!m_vif.has_write_strobes) begin
        ...
        if (shifted_wstrb != '0 && shifted_wstrb != strb_mask_for_size) begin
          `uvm_error(get_full_name(),
                     $sformatf({"Cannot represent m_wstrb = 0x%0h on this interface. ",
                                "Because Write_Strobes is false, there is no hwstrb signal and ",
                                "m_size and m_addr for this item give a relevant 'true' strobe ",
                                "mask of 0x%0h. After being shifted by the address, m_wstrb is ",
                                "0x%0h, which has neither all the relevant bits nor zero."},
                               m_cur_data_phase.m_wstrb, relevant_strb_mask, shifted_wstrb))
        end
      end
    end

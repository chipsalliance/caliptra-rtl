...
Caliptra RTL repository uses the squash strategy to merge changes from a Pull Request to the destination branch. This means that the title of the PR becomes the subject of the commit in the destination branch's history. To improve debug, triage, readability, and preparation of change-lists, it is imperative that these commit subject lines be concise, meaningful, and consistent. The following conventions are used:

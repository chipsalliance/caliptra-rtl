...
class otp_ctrl_scoreboard #(type CFG_T = otp_ctrl_env_cfg)
    ...
    case (csr_name)
      ...
      "direct_access_cmd": begin
        if (addr_phase_write && !cfg.otp_ctrl_vif.under_error_states()) begin
          ...
          end else if (part_idx == NonSecretFusesIdx) begin
            sw_read_lock = `gmv(ral.non_secret_fuses_read_lock) == 0;
          end
      ...
      "non_secret_fuses_digest_0", "non_secret_fuses_digest_1",
      "secret0_digest_0", "secret0_digest_1",
      "secret1_digest_0", "secret1_digest_1": begin
        if (ignore_digest_chk) do_read_check = 0;
      end
      "vendor_test_read_lock",
      "non_secret_fuses_read_lock",
  ...
  virtual function void predict_digest_csrs();
    ...
    void'(ral.non_secret_fuses_digest[0].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[NonSecretFusesIdx]]),
          .kind(UVM_PREDICT_DIRECT)));
    void'(ral.non_secret_fuses_digest[1].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[NonSecretFusesIdx] + 1]),
          .kind(UVM_PREDICT_DIRECT)));
  ...
  virtual function bit [TL_DW*2-1:0] get_digest_reg_val(int part_idx);
    ...
    case (part_idx)
      ...
      NonSecretFusesIdx: begin
        digest = {`gmv(ral.non_secret_fuses_digest[1]),
                  `gmv(ral.non_secret_fuses_digest[0])};
      end
      Secret0Idx: begin
  ...
  virtual function bit is_tl_mem_access_allowed(input tl_seq_item item, input string ral_name,
    ...
    if (addr inside {
      ...
      if (`gmv(ral.non_secret_fuses_read_lock) == 0 ||
          cfg.otp_ctrl_vif.under_error_states()) begin
        if (addr inside {
            [cfg.ral_models[ral_name].mem_ranges[0].start_addr + NonSecretFusesOffset :
             cfg.ral_models[ral_name].mem_ranges[0].start_addr + NonSecretFusesOffset +
             NonSecretFusesSize - 1]}) begin
          predict_err(OtpNonSecretFusesErrIdx, OtpAccessError);
          custom_err = 1;
          if (cfg.en_cov) begin
            cov.unbuf_access_lock_cg_wrap[NonSecretFusesIdx].sample(.read_lock(1),
                .write_lock(get_digest_reg_val(NonSecretFusesIdx) != 0), .is_write(0));
          end
          return 0;
        end
      end

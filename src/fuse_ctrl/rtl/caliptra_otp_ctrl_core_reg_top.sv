...
module caliptra_otp_ctrl_core_reg_top (
  ...
  */
 input logic                                      win_dv,
 input logic   [AW-1:0]                           win_addr,
 input logic                                      win_write,
 //input logic   [UW-1:0]                           win_user,
 input logic   [IW-1:0]                           win_id,
 input logic   [DW-1:0]                           win_wdata,
 input logic   [BC-1:0]                           win_wstrb,
 output logic  [DW-1:0]                           win_rdata,
 input logic                                      win_last,
 output logic                                     win_hld,
  ...
  */
  ...
  always_comb begin
    ...
    addr_hit[13] = (reg_addr == CALIPTRA_OTP_CTRL_DIRECT_ACCESS_REGWEN_OFFSET);
    addr_hit[14] = (reg_addr == CALIPTRA_OTP_CTRL_DIRECT_ACCESS_CMD_OFFSET);
    addr_hit[15] = (reg_addr == CALIPTRA_OTP_CTRL_DIRECT_ACCESS_ADDRESS_OFFSET);
    addr_hit[16] = (reg_addr == CALIPTRA_OTP_CTRL_DIRECT_ACCESS_WDATA_0_OFFSET);
    addr_hit[17] = (reg_addr == CALIPTRA_OTP_CTRL_DIRECT_ACCESS_WDATA_1_OFFSET);
    addr_hit[18] = (reg_addr == CALIPTRA_OTP_CTRL_DIRECT_ACCESS_RDATA_0_OFFSET);
    addr_hit[19] = (reg_addr == CALIPTRA_OTP_CTRL_DIRECT_ACCESS_RDATA_1_OFFSET);
    addr_hit[20] = (reg_addr == CALIPTRA_OTP_CTRL_CHECK_TRIGGER_REGWEN_OFFSET);
    addr_hit[21] = (reg_addr == CALIPTRA_OTP_CTRL_CHECK_TRIGGER_OFFSET);
    addr_hit[22] = (reg_addr == CALIPTRA_OTP_CTRL_CHECK_REGWEN_OFFSET);
    addr_hit[23] = (reg_addr == CALIPTRA_OTP_CTRL_CHECK_TIMEOUT_OFFSET);
    addr_hit[24] = (reg_addr == CALIPTRA_OTP_CTRL_INTEGRITY_CHECK_PERIOD_OFFSET);
    addr_hit[25] = (reg_addr == CALIPTRA_OTP_CTRL_CONSISTENCY_CHECK_PERIOD_OFFSET);
    addr_hit[26] = (reg_addr == CALIPTRA_OTP_CTRL_VENDOR_TEST_READ_LOCK_OFFSET);
    ...
    addr_hit[29] = (reg_addr == CALIPTRA_OTP_CTRL_VENDOR_TEST_DIGEST_1_OFFSET);
    addr_hit[30] = (reg_addr == CALIPTRA_OTP_CTRL_NON_SECRET_FUSES_DIGEST_0_OFFSET);
    addr_hit[31] = (reg_addr == CALIPTRA_OTP_CTRL_NON_SECRET_FUSES_DIGEST_1_OFFSET);
    addr_hit[32] = (reg_addr == CALIPTRA_OTP_CTRL_SECRET0_DIGEST_0_OFFSET);
    addr_hit[33] = (reg_addr == CALIPTRA_OTP_CTRL_SECRET0_DIGEST_1_OFFSET);
    addr_hit[34] = (reg_addr == CALIPTRA_OTP_CTRL_SECRET1_DIGEST_0_OFFSET);
    addr_hit[35] = (reg_addr == CALIPTRA_OTP_CTRL_SECRET1_DIGEST_1_OFFSET);
    addr_hit[36] = (reg_addr == CALIPTRA_OTP_CTRL_SECRET2_DIGEST_0_OFFSET);
    addr_hit[37] = (reg_addr == CALIPTRA_OTP_CTRL_SECRET2_DIGEST_1_OFFSET);
  end
  ...
  assign direct_access_regwen_re = addr_hit[13] & reg_re & !reg_error;
  assign direct_access_regwen_we = addr_hit[13] & reg_we & !reg_error;

  assign direct_access_regwen_wd = reg_wdata[0];
  assign direct_access_cmd_we = addr_hit[14] & reg_we & !reg_error;
  ...
  assign direct_access_address_we = addr_hit[15] & reg_we & !reg_error;

  assign direct_access_address_wd = reg_wdata[11:0];
  assign direct_access_wdata_0_we = addr_hit[16] & reg_we & !reg_error;

  assign direct_access_wdata_0_wd = reg_wdata[31:0];
  assign direct_access_wdata_1_we = addr_hit[17] & reg_we & !reg_error;

  assign direct_access_wdata_1_wd = reg_wdata[31:0];
  assign direct_access_rdata_0_re = addr_hit[18] & reg_re & !reg_error;
  assign direct_access_rdata_1_re = addr_hit[19] & reg_re & !reg_error;
  assign check_trigger_regwen_we = addr_hit[20] & reg_we & !reg_error;

  assign check_trigger_regwen_wd = reg_wdata[0];
  assign check_trigger_we = addr_hit[21] & reg_we & !reg_error;

  assign check_trigger_integrity_wd = reg_wdata[0];

  assign check_trigger_consistency_wd = reg_wdata[1];
  assign check_regwen_we = addr_hit[22] & reg_we & !reg_error;

  assign check_regwen_wd = reg_wdata[0];
  assign check_timeout_we = addr_hit[23] & reg_we & !reg_error;

  assign check_timeout_wd = reg_wdata[31:0];
  assign integrity_check_period_we = addr_hit[24] & reg_we & !reg_error;

  assign integrity_check_period_wd = reg_wdata[31:0];
  assign consistency_check_period_we = addr_hit[25] & reg_we & !reg_error;

  assign consistency_check_period_wd = reg_wdata[31:0];
  assign vendor_test_read_lock_we = addr_hit[26] & reg_we & !reg_error;

  assign vendor_test_read_lock_wd = reg_wdata[0];
  assign non_secret_fuses_read_lock_we = addr_hit[27] & reg_we & !reg_error;

  assign non_secret_fuses_read_lock_wd = reg_wdata[0];
  assign vendor_test_digest_0_re = addr_hit[28] & reg_re & !reg_error;
  assign vendor_test_digest_1_re = addr_hit[29] & reg_re & !reg_error;
  assign non_secret_fuses_digest_0_re = addr_hit[30] & reg_re & !reg_error;
  assign non_secret_fuses_digest_1_re = addr_hit[31] & reg_re & !reg_error;
  assign secret0_digest_0_re = addr_hit[32] & reg_re & !reg_error;
  assign secret0_digest_1_re = addr_hit[33] & reg_re & !reg_error;
  assign secret1_digest_0_re = addr_hit[34] & reg_re & !reg_error;
  assign secret1_digest_1_re = addr_hit[35] & reg_re & !reg_error;
  assign secret2_digest_0_re = addr_hit[36] & reg_re & !reg_error;
  assign secret2_digest_1_re = addr_hit[37] & reg_re & !reg_error;
  ...
  always_comb begin
    ...
    reg_we_check[13] = direct_access_regwen_we;
    reg_we_check[14] = direct_access_cmd_gated_we;
    reg_we_check[15] = direct_access_address_gated_we;
    reg_we_check[16] = direct_access_wdata_0_gated_we;
    reg_we_check[17] = direct_access_wdata_1_gated_we;
    reg_we_check[18] = 1'b0;
    reg_we_check[19] = 1'b0;
    reg_we_check[20] = check_trigger_regwen_we;
    reg_we_check[21] = check_trigger_gated_we;
    reg_we_check[22] = check_regwen_we;
    reg_we_check[23] = check_timeout_gated_we;
    reg_we_check[24] = integrity_check_period_gated_we;
    reg_we_check[25] = consistency_check_period_gated_we;
    reg_we_check[26] = vendor_test_read_lock_gated_we;
    reg_we_check[27] = non_secret_fuses_read_lock_gated_we;
  ...
  always_comb begin
    ...
    unique case (1'b1)
      ...
      addr_hit[4]: begin
        ...
        reg_rdata_next[5] = status_life_cycle_error_qs;
        reg_rdata_next[6] = status_dai_error_qs;
        reg_rdata_next[7] = status_lci_error_qs;
        reg_rdata_next[8] = status_timeout_error_qs;
        reg_rdata_next[9] = status_lfsr_fsm_error_qs;
        reg_rdata_next[10] = status_scrambling_fsm_error_qs;
        reg_rdata_next[11] = status_key_deriv_fsm_error_qs;

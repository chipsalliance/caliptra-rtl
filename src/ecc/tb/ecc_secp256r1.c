 ...
 */
#define ECPARAMS    MBEDTLS_ECP_DP_SECP256R1  MBEDTLS_ECDSA_DETERMINISTIC
...
{
    //mbedtls_printf("Caliptra testvector generator for ECDSA secp256r1");
    int ret = 1;
    ...
    unsigned char z_buf[32];
    size_t z_buf_len  = sizeof(z_buf);
    mbedtls_hmac_drbg_context rng_ctx;
    mbedtls_hmac_drbg_context *p_rng = &rng_ctx;

    uint8_t privkey_buf[32];
    size_t privkey_buf_len = sizeof(privkey_buf);
    unsigned char pubkeyX_buf[65];
    size_t pubkeyX_buf_len = sizeof(pubkeyX_buf);
    uint8_t pubkeyY_buf[32];
    size_t pubkeyY_buf_len = sizeof(pubkeyY_buf);

    unsigned char pubkey_test[96]; //TODO: what is this? is 96 right? 32*3 vs 48*3 for secp256r1

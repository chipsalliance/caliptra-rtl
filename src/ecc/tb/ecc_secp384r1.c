...
int main( int argc, char *argv[] )
{
     ...
     * HMAC-DRBG seeded with entropy=seed_buf, nonce=nonce_buf
     * (mirrors KEYGEN seeding).  Output to a separate file so the
     * deterministic KAT set is untouched. */
    fptr = fopen("secp384_nondet_testvector.hex", "w");
#else
    ...
    FILE *fptr_all;
#ifdef NON_DET
    fptr_all = fopen("secp384_nondet_testvector_all.hex", "a");
#else

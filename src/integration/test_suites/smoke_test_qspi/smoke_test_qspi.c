...
void end_sim_if_qspi_disabled() {
  ...
  if (hw_cfg & SOC_IFC_REG_CPTRA_HW_CONFIG_QSPI_EN_MASK) {
    VPRINTF_LOW("Internal QSPI is enabled, running QSPI smoke test\n");
  } else {
    VPRINTF_FATAL("Internal QSPI is not enabled, skipping QSPI smoke test\n");
...
void spi_command(int length, int csaat, speed_t speed, direction_t direction) {
  ...
  VPRINTF_LOW("  - Writing spi_command: len:%d csaat:%d speed:%d direction:%d\n",
...
void configure_spi_host(int host) {
  ...
  VPRINTF_LOW("Configuring spi_host[%d]\n", host);
...
int run_jedec_id_test(int host) {
  ...
  VPRINTF_LOW("  - reading data from device...\n");

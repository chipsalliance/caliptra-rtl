...
void aes_flow_w_error_expected(aes_op_e op, aes_mode_e mode, aes_key_len_e key_len, aes_flow_t aes_input, aes_endian_e endian_mode, uint8_t error_expected){
  ...
  if(error_expected){
    VPRINTF(LOW, "Error_expected flag is set, data will not be checked.\n");
  }
  ...
  if (aes_input.text_len > 0) { 
    if (aes_input.data_src_mode == AES_DATA_DMA){
        ...
        soc_ifc_axi_dma_send_axi_to_axi_w_error_expected(aes_input.dma_transfer_data.src_addr, 0, aes_input.dma_transfer_data.dst_addr, 0,  aes_input.text_len, 0, 1, gcm_mode, error_expected);

        soc_ifc_axi_dma_read_ahb_payload_w_error_expected(aes_input.dma_transfer_data.dst_addr, 0, read_payload, aes_input.text_len, 0, error_expected);

...
void init_interrupts(void) {
    ...
    *mpiccfg = 0x0; // 0x0 - Standard compliant priority order: 0=lowest,15=highest
    ...
    abr_reg[ABR_REG_INTR_BLOCK_RF_NOTIF_INTR_EN_R /sizeof(uint32_t)] = ABR_REG_INTR_BLOCK_RF_NOTIF_INTR_EN_R_NOTIF_CMD_DONE_EN_MASK;
    abr_reg[ABR_REG_INTR_BLOCK_RF_GLOBAL_INTR_EN_R/sizeof(uint32_t)] = ABR_REG_INTR_BLOCK_RF_GLOBAL_INTR_EN_R_ERROR_EN_MASK |
                                                                       ABR_REG_INTR_BLOCK_RF_GLOBAL_INTR_EN_R_NOTIF_EN_MASK;
...
// Non-Standard Vectored Interrupt Handler (ABR Error = vector 23)
nonstd_veer_isr(abr_error)
// Non-Standard Vectored Interrupt Handler (ABR Notification = vector 24)
nonstd_veer_isr(abr_notif)

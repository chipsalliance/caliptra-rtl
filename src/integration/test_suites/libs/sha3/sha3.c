...
void dif_kmac_customization_string_init(
  ...
  if ((data == NULL && len != 0) || out == NULL) {
    VPRINTF(LOW, "dif_kmac_customization_string_init: ERROR data and out arguments must not be null.\n");
    while (1);
    return;
  }

  if (len > kDifKmacMaxCustomizationStringLen) {
    VPRINTF(LOW, "dif_kmac_customization_string_init: ERROR length greater than maximum.\n");
    while (1);
    return;
  }

  if (kDifKmacMaxCustomizationStringLen > UINT16_MAX / 8) {
    VPRINTF(LOW, "dif_kmac_customization_string_init: ERROR length requires more than 3 bytes to left encode.\n");
    while (1);
    return;
  }
  if ((sizeof(out->buffer) / sizeof(char)) < kDifKmacMaxCustomizationStringLen + 3) {
    VPRINTF(LOW, "dif_kmac_customization_string_init: ERROR buffer is not large enough\n");
    while (1);
    return;
  }
...
void dif_kmac_function_name_init(const char *data, size_t len, dif_kmac_function_name_t *out) {
  if ((data == NULL && len != 0) || out == NULL) {
    VPRINTF(LOW, "dif_kmac_function_name_init: ERROR data and out must not be NULL.\n");
    while (1);
    return;
  }

  if (len > kDifKmacMaxFunctionNameLen) {
    VPRINTF(LOW, "dif_kmac_function_name_init: ERROR length larger than maximum.\n");
    while (1);
    return;
  }

  if (kDifKmacMaxFunctionNameLen > UINT8_MAX / 8) {
    VPRINTF(LOW, "dif_kmac_function_name_init: ERROR length requires more than 2 bytes to left encode\n");
    while (1);
    return;
  }
  if ((sizeof(out->buffer) / sizeof(char)) < kDifKmacMaxFunctionNameLen + 2) {
    VPRINTF(LOW, "dif_kmac_function_name_init: ERROR buffer is not large enough.\n");
    while (1);
    return;
  }
...
void dif_kmac_mode_sha3_start(
  ...
  if (kmac == 0 || operation_state == 0) {
    VPRINTF(LOW, "dif_kmac_mode_sha3_start: ERROR kmac or operation_state NULL.\n");
    while(1);
    return;
  }
  ...
  switch (mode) {
    ...
    case kDifKmacModeSha3Len512:
      ...
      break;
    default:
      VPRINTF(LOW, "dif_kmac_sha3_start: ERROR Unsupported mode.\n");
      while(1);
      return;
  }
  ...
  if ((kmac_status & (0x1U << KMAC_STATUS_SHA3_IDLE_INDEX)) == 0) {
    VPRINTF(LOW, "dif_kmac_sha3_start: ERROR hardware must be idle.\n");
    while(1);
    return;
  }
...
void dif_kmac_mode_shake_start(
  ...
  if (kmac == 0 || operation_state == NULL) {
    VPRINTF(LOW, "dif_kmac_mode_shake_start: ERROR kmac and operation state cannot be NULL.\n");
    while (1);
    return;
  }
  ...
  switch (mode) {
    ...
    case kDifKmacModeShakeLen256:
      ...
      break;
    default:
      VPRINTF(LOW, "dif_kmac_mode_shake_start: ERROR mode not allowed.\n");
      while (1);
      return;
  }
  ...
  uint32_t kmac_status = lsu_read_32(kmac + KMAC_STATUS_REG_OFFSET);
  if ((kmac_status & (0x1U << KMAC_STATUS_SHA3_IDLE_INDEX)) == 0) {
    VPRINTF(LOW, "dif_kmac_shake_start: ERROR hardware must be idle.\n");
    while(1);
    return;
  }
  operation_state->squeezing = false;
...
void dif_kmac_mode_cshake_start(
  ...
  if (kmac == 0 || operation_state == NULL) {
    VPRINTF(LOW, "dif_kmac_mode_cshake_start: ERROR kmac or operation state is NULL.\n");
    while (1);
    return;
  }
  ...
  if (n_is_empty && s_is_empty) {
    switch (mode) {
      ...
      case kDifKmacModeCshakeLen256:
        ...
        return;
      default:
        VPRINTF(LOW, "dif_kmac_mode_cshake_start: ERROR unsupported mode for empty N and S.\n");
        while (1);
        return;
    }
  }
  ...
  switch (mode) {
    ...
    case kDifKmacModeCshakeLen256:
      ...
      break;
    default:
      VPRINTF(LOW, "dif_kmac_mode_cshake_start: ERROR unsupported kstrenght.\n");
      while (1);
      return;
  }
  ...
  uint32_t kmac_status = lsu_read_32(kmac + KMAC_STATUS_REG_OFFSET);
  if ((kmac_status & (0x1U << KMAC_STATUS_SHA3_IDLE_INDEX)) == 0) {
    VPRINTF(LOW, "dif_kmac_mode_cshake_start: ERROR hardware must be idle.\n");
    while (1);
    return;
  }
  operation_state->squeezing = false;
...
void dif_kmac_absorb(
  ...
  if (kmac == 0 || operation_state == 0 || (msg == 0 && len != 0)) {
    VPRINTF(LOW, "dif_kmac_absorb: ERROR one of function arguments is null\n");
    while (1);
    return;
  }
  ...
  if (operation_state->r == 0) {
    VPRINTF(LOW, "dif_kmac_absorb: ERROR operation not started yet\n");
    while (1);
    return;
  }
  ...
  if ((kmac_status & (0x1U << KMAC_STATUS_SHA3_ABSORB_INDEX)) == 0) {
    VPRINTF(LOW, "dif_kmac_absorb: ERROR hardware must be absorbing.\n");
    while(1);
    return;
  }
...
void dif_kmac_squeeze(
  ...
  if (kmac == 0 || operation_state == 0 || (out == 0 && len != 0)) {
    VPRINTF(LOW, "dif_kmac_squeeze: ERROR arguments may not be NULL.\n");
    while (1);
    return;
  }
  ...
  if (operation_state->d != 0 &&
    ...
    VPRINTF(LOW, "dif_kmac_squeeze: ERROR total bytes requested must not exceed fixed output length.\n");
    while (1);
    return;
  }
    ...
    if (n == 0) {
      ...
      if (operation_state->d != 0) {
        if (operation_state->d <= operation_state->r) {
          VPRINTF(LOW, "dif_kmac_squeeze: ERROR operation state d less than r.\n");
          while (1);
          return;
        }
...
void dif_kmac_end(
    const uintptr_t kmac, dif_kmac_operation_state_t *operation_state) {
  if (kmac == 0 || operation_state == 0) {
    VPRINTF(LOW, "dif_kmac_end: ERROR arguments may not be NULL.\n");
    while (1);
    return;
  }
  ...
  // command is issued.
  if (!operation_state->squeezing) {
    VPRINTF(LOW, "dif_kmac_end: ERROR hardware must be done squeezing.\n");
    while (1);
    return;
  }
  while (true) {

...
void main(void) {
        ...
        if (data != CALIPTRA_HW_REV_ID) {
            VPRINTF_FATAL("HW REV ID register reports a version of major [%u], minor [%u], patch [%u] which does not match the expected value %u.%u.%u\n",
                           (data & 0xf),
                           (data & 0xf0) >> 4,
                           (data & 0xff00) >> 8,
        ...
        #else
        VPRINTF_FATAL("CALIPTRA_HW_REV_ID is not defined!\n");
        fail = 1;
        #endif
        ...
        if ((data & SOC_IFC_REG_CPTRA_HW_CONFIG_ITRNG_EN_MASK) != data_exp) {
            VPRINTF_FATAL("HW Config register reports [%d] for ITRNG EN, expected [%d]!\n", (data & SOC_IFC_REG_CPTRA_HW_CONFIG_ITRNG_EN_MASK), data_exp);
            fail = 1;
        }
        ...
        if ((data & SOC_IFC_REG_CPTRA_HW_CONFIG_LMS_ACC_EN_MASK) != data_exp) {
            VPRINTF_FATAL("HW Config register reports [%d] for LMS Acc EN, expected [%d]!\n", (data & SOC_IFC_REG_CPTRA_HW_CONFIG_LMS_ACC_EN_MASK), data_exp);
            fail = 1;
        }
        ...
        if ((data & SOC_IFC_REG_CPTRA_HW_CONFIG_SUBSYSTEM_MODE_EN_MASK) != data_exp) {
            VPRINTF_FATAL("HW Config register reports [%d] for SUBSYSTEM MODE, expected [%d]!\n", (data & SOC_IFC_REG_CPTRA_HW_CONFIG_SUBSYSTEM_MODE_EN_MASK), data_exp);
            fail = 1;
        }

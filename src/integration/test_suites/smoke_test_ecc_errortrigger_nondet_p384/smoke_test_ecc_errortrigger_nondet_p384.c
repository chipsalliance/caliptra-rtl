...
//
// P-384 half of nondet errortrigger (SIGN-only error paths with RAND_K_EN=1).
...
// Helper: run a P-384 SIGN+RAND_K_EN subtest with the given privkey, expect ecc_error.
static void run_p384_nondet_sign_err(const uint32_t *privkey, uint8_t inject_cmd,
                                     const char *label) {
    while ((lsu_read_32(CLP_ECC_REG_ECC_STATUS) & ECC_REG_ECC_STATUS_READY_MASK) == 0);

    VPRINTF(LOW, "\n %s (CURVE_SEL=0, RAND_K_EN=1)\n", label);

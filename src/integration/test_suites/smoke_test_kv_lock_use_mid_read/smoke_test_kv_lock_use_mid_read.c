...
void main() {
    ...
    for (uint8_t test_slot = 0; test_slot < 24; ++test_slot) {
        ...
        //
        // TB command 0x807F (with slot encoded in bits [12:8]) injects hmac512_key_tb into the slot matched by
        //   ((cmd & 0x1f00) >> 8) == slot_id, with:
        //   dest_valid = 5'b1  (bit 0 = HMAC_KEY read client allowed)
        //   last_dword = 15    (16 dwords)
        // Slot is encoded in the [12:8] bits of the command, so correct command
        // encoding looks like: 0x80A3 | (slot << 8).
        // ------------------------------------------------------------------
        VPRINTF(LOW, "[SETUP] Injecting HMAC512 key into KV slot %d via TB...\n", test_slot);
        lsu_write_32(CLP_SOC_IFC_REG_CPTRA_GENERIC_OUTPUT_WIRES_0, (uint32_t)(0x80A3 | (test_slot << 8)));

...
//
// Curve-switch smoke (part 1 of 2): KEYGEN + SIGN det.
// Sequence: KEYGEN P-384 -> KEYGEN P-256 -> SIGN P-384 det -> SIGN P-256 det.
// 3 back-to-back CURVE_SEL transitions, no ecc_zeroize() in between, all
// deterministic outputs checked against KATs. Validates curve_sel_reg
// latching and no cross-curve datapath contamination for KEYGEN/SIGN-det
// paths. Companion: smoke_test_ecc_curve_switch_nondet_sign_verify (SIGN nondet
// + VERIFY).
...
// P-384 KAT (smoke_test_ecc_sign / smoke_test_ecc_keygen vectors).
...
// P-256 KAT (smoke_test_ecc_keygen_p256 / sign_p256 vectors). Upper 4 dwords zero.
...
// Fill an ecc_io with register-sourced data from a 12-dword vector.
...
void main() {
    VPRINTF(LOW, "----------------------------------------------------\n");
    VPRINTF(LOW, " ECC curve-switch (KEYGEN+SIGN det) smoke test !!\n");
    VPRINTF(LOW, "----------------------------------------------------\n");

    init_interrupts();

    ecc_io seed, nonce, iv, privkey, pubkey_x, pubkey_y, msg, sign_r, sign_s;
    ...
    // [2/4] KEYGEN P-256 (P-384 -> P-256 switch)
    VPRINTF(LOW, "\n[2/4] KEYGEN P-256\n");
    make_io(&seed,     p256_seed);
    make_io(&nonce,    p256_nonce);
    make_io(&iv,       p256_iv);
    make_io(&privkey,  p256_privkey);
    make_io(&pubkey_x, p256_pubkey_x);
    make_io(&pubkey_y, p256_pubkey_y);
    ecc_keygen_flow(seed, nonce, iv, privkey, pubkey_x, pubkey_y, TRUE, 1);
    cptra_intr_rcv.ecc_notif = 0;

    // [3/4] SIGN P-384 det (P-256 -> P-384 back-switch)
    VPRINTF(LOW, "\n[3/4] SIGN P-384 det\n");
    make_io(&privkey, p384_privkey);
    ...
    // [4/4] SIGN P-256 det (P-384 -> P-256 switch)
    VPRINTF(LOW, "\n[4/4] SIGN P-256 det\n");
    make_io(&privkey, p256_privkey);

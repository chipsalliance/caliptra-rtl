...
inline void service_mldsa_error_intr() {
    ...
    if (sts & MLDSA_REG_INTR_BLOCK_RF_ERROR_INTERNAL_INTR_R_ERROR_INTERNAL_STS_MASK) {
        *reg = MLDSA_REG_INTR_BLOCK_RF_ERROR_INTERNAL_INTR_R_ERROR_INTERNAL_STS_MASK;
        cptra_intr_rcv.mldsa_error |= MLDSA_REG_INTR_BLOCK_RF_ERROR_INTERNAL_INTR_R_ERROR_INTERNAL_STS_MASK;
    }
    if (sts == 0) {
        VPRINTF(ERROR,"bad mldsa_error_intr sts:%x\n", sts);

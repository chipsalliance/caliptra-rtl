...
inline void service_hmac256_error_intr() {
    uint32_t * reg = (uint32_t *) (CLP_HMAC256_REG_INTR_BLOCK_RF_ERROR_INTERNAL_INTR_R);
    uint32_t sts = *reg;
    if (sts != 0) {
        *reg = sts;
        cptra_intr_rcv.hmac256_error |= sts;
    } else {
...
inline void service_hmac256_notif_intr() {
    uint32_t * reg = (uint32_t *) (CLP_HMAC256_REG_INTR_BLOCK_RF_NOTIF_INTERNAL_INTR_R);
    uint32_t sts = *reg;

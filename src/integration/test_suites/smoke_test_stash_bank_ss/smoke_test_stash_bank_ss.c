...
void main(void) {
    ...
    // Step B: subsystem mode implements slot 0 only. The BFM wrote all eight lock
    // bits, so this check also covers bits 7:1 being unimplemented rather than
    // merely unused - a locked-looking slot that can never hold data would let
    // the drainer extend PCR31 with zeros.
    ...
    VPRINTF(LOW, "FW: slot_locked = 0x%02x (BFM wrote SOC_LOCK=0xFF; bits 7:1 unimplemented)\n",
            slot_locked_mask);
    ...
    // Step F: set CPTRA_LOCK (per RFC 673 §4.4 / §7.3).

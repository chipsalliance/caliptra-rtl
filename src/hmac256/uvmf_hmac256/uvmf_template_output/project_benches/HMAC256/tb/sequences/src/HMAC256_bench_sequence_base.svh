...
class HMAC256_bench_sequence_base extends uvmf_sequence_base #(uvm_sequence_item);

  `uvm_object_utils( HMAC256_bench_sequence_base );

  // pragma uvmf custom sequences begin

typedef HMAC256_env_sequence_base #(
        .CONFIG_T(hmac256_env_configuration_t)
        )
        hmac256_env_sequence_base_t;
rand hmac256_env_sequence_base_t hmac256_env_seq;



  // UVMF_CHANGE_ME : Instantiate, construct, and start sequences as needed to create stimulus scenarios.
  // Instantiate sequences here
  typedef HMAC256_rst_random_sequence  hmac256_rst_agent_random_seq_t;
  hmac256_rst_agent_random_seq_t hmac256_rst_agent_random_seq;
  // pragma uvmf custom sequences end

  // Sequencer handles for each active interface in the environment
  typedef HMAC256_rst_transaction  hmac256_rst_agent_transaction_t;
  uvm_sequencer #(hmac256_rst_agent_transaction_t)  hmac256_rst_agent_sequencer; 

  // Sequencer handles for each QVIP interface
  mvc_sequencer uvm_test_top_environment_qvip_ahb_lite_slave_subenv_ahb_lite_slave_0_sqr;

  // Top level environment configuration handle
  hmac256_env_configuration_t top_configuration;

  // Configuration handles to access interface BFM's
  HMAC256_rst_configuration  hmac256_rst_agent_config;
  // Local handle to register model for convenience
  hmac256_reg_model_top reg_model;
  uvm_status_e status;
  ...
  // Number of dwords in one HMAC256 raw SHA block (32 * 32-bit = 512 bits).
  localparam int unsigned BENCH_BLOCK_DWORDS = 16;

  // ----------------------------------------------------------------
  // Poll HMAC256_STATUS until any bit in mask_bit is set. Bounded by
  ...
  task wait_for_status(input bit [31:0] mask_bit,
    ...
    int poll_count = 0;
    do begin
      reg_model.HMAC256_STATUS.read(status, data);
      poll_count++;
      if (poll_count > BENCH_POLL_LIMIT) begin
        `uvm_fatal("HMAC256_BENCH",
  ...
  // loops over HMAC256_{KEY,BLOCK,TAG,LFSR_SEED} so sequence bodies
  ...
  task read_tag_regs(output bit [31:0] tag [8]);
    foreach (reg_model.HMAC256_TAG[i])
      reg_model.HMAC256_TAG[i].read(status, tag[i]);
  endtask

  task write_tag_regs(input bit [31:0] tag [8]);
    foreach (reg_model.HMAC256_TAG[i])
      reg_model.HMAC256_TAG[i].write(status, tag[i]);
  endtask
  ...
  // total block_length B (so the HMAC256 core hashes K_ipad + B-1 msg
  // blocks + 1 pad block); pad_len_bits = B * 512.
  // ----------------------------------------------------------------
  function void build_pad_block(input int unsigned block_length,
                                output bit [31:0] blk [BENCH_BLOCK_DWORDS]);
    blk[0] = 32'h8000_0000;
    for (int i = 1; i < 15; i++) blk[i] = 32'h0;
    blk[15] = block_length * 512;
  endfunction

  // ----------------------------------------------------------------
  // Drive blocks[start_idx..end_idx] for one HMAC256 op.
  ...
  function new( string name = "" );
    ...
    if ( !uvm_config_db#(hmac256_env_configuration_t)::get(null,UVMF_CONFIGURATIONS, "TOP_ENV_CONFIG",top_configuration) ) begin
      `uvm_info("CFG", "*** FATAL *** uvm_config_db::get can not find TOP_ENV_CONFIG.  Are you using an older UVMF release than what was used to generate this bench?",UVM_NONE);
      `uvm_fatal("CFG", "uvm_config_db#(hmac256_env_configuration_t)::get cannot find resource TOP_ENV_CONFIG");
    end

    // Retrieve config handles for all agents
    if( !uvm_config_db #( HMAC256_rst_configuration )::get( null , UVMF_CONFIGURATIONS , hmac256_rst_agent_BFM , hmac256_rst_agent_config ) ) 
      `uvm_fatal("CFG" , "uvm_config_db #( HMAC256_rst_configuration )::get cannot find resource hmac256_rst_agent_BFM" )

    // Assign the sequencer handles from the handles within agent configurations
    hmac256_rst_agent_sequencer = hmac256_rst_agent_config.get_sequencer();
    ...
    reg_model = top_configuration.hmac256_rm;
  ...
  virtual task body();
    ...
    hmac256_env_seq = hmac256_env_sequence_base_t::type_id::create("hmac256_env_seq");

    hmac256_rst_agent_random_seq     = hmac256_rst_agent_random_seq_t::type_id::create("hmac256_rst_agent_random_seq");
    fork
...
hmac256_env_seq.start(top_configuration.vsqr);

    // UVMF_CHANGE_ME : Extend the simulation XXX number of clocks after 
    // the last sequence to allow for the last sequence item to flow 
    // through the design.
    fork
      hmac256_rst_agent_config.wait_for_num_clocks(400);
    join

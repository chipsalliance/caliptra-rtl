...
// hmac256_core.sv
// ------------
// HMAC-SHA-256/224 core with SCA countermeasures.
// Single SHA-256 masked core architecture for area optimization.
//
// Commands:
...
//               the HMAC tag.
//   restore_cmd - Resume processing from a previously captured inner
//                 chaining state (passed in via restore_digest) instead
//                 of starting from H0; combined with next_cmd or
//                 next_cmd|last_cmd.
//
//======================================================================
...
(
  ...
  //----------------------------------------------------------------
  // IPAD/OPAD and SHA-256 family final-padding constants come from
  // hmac256_param_pkg so the same widths/values are used everywhere.
    ...
    begin
      if (!reset_n)
        ...
        if (hmac_ctrl_reg == CTRL_IDLE)
          mode_reg <= mode_cmd;  // 0 for HMAC-SHA-224, 1 for HMAC-SHA-256
      end
    end

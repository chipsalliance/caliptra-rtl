...
//----------------------------------------------------------------------
// Description: HMAC_save_restore_sequence
//   Models the real save / restore use case: a long HMAC op gets
...
class HMAC_save_restore_sequence extends HMAC_bench_sequence_base;

  `uvm_object_utils(HMAC_save_restore_sequence)
  ...
  bit [31:0] key_a       [16];
  bit [31:0] blocks_a    [];   // sized to block_length_a * 32
  bit [31:0] ref_tag_a   [16];
  bit [31:0] save_tag    [16];
  bit [31:0] restore_tag [16];

  // Context B (the intervening op).
  bit [31:0] key_b       [16];
  bit [31:0] blocks_b    [];   // sized to block_length_b * 32
  bit [31:0] ref_tag_b   [16];
  bit [31:0] mid_tag_b   [16];

  uvm_event ev_rst;

  function new(string name = "HMAC_save_restore_sequence");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    if (!this.randomize())
      `uvm_fatal("HMAC_SR", "randomize() failed -- check c_a_len/c_save/c_b_len")
    ...
    `uvm_info("HMAC_SR", "Step 2: snapshot intermediate state of A", UVM_LOW)
    ev_rst.trigger();
    fork hmac_rst_agent_config.wait_for_num_clocks(50); join
    reg_model.reset();
    ...
    `uvm_info("HMAC_SR",
      $sformatf("Saved A.TAG[0]=0x%08h A.TAG[15]=0x%08h",
                save_tag[0], save_tag[15]), UVM_LOW)

    // -------- Step 3: intervening HMAC for context B --------
    `uvm_info("HMAC_SR", "Step 3: intervening HMAC for context B", UVM_LOW)
    wait_for_status(32'h1, "READY", read_data);
    ...
    if (mismatches_b == 0)
      `uvm_info("HMAC_SR_CHECK",
        "Intervening B op self-check passed", UVM_LOW)

    // -------- Step 4: restore context A and finish MSG1 --------
    `uvm_info("HMAC_SR", "Step 4: restore context A and finish MSG1", UVM_LOW)
    ev_rst.trigger();
    fork hmac_rst_agent_config.wait_for_num_clocks(50); join
    reg_model.reset();

...
`ifdef CALIPTRA_MODE_SUBSYSTEM
                    //Subsystem mode: PCR4/PCR5 are reserved for the ICCM hash engine
                    //(pv_write[1]). pv_write[0] (sha512_ctrl) is blocked by RTL in
                    //pv.sv from writing these entries, so drop the mirror update
                    //to keep prediction aligned with the DUT.
...
`else
                    //Non-subsystem: no PCR4/PCR5 write guard in RTL, mirror all writes.

...
| :--------- | :--------- | :--------- |
| Obf-Key-1                  | SoC backend flows shall generate obfuscation key with appropriate NIST compliance as dictated in the Caliptra RoT specification.                                                                                                                                               | Required by UDS and Field Entropy threat model    |
| Obf-Key-2                  | If not driven through PUF, SoC backend flows shall ECO the obfuscation key before tapeout.                                                                                                                                                                                     | Required by UDS and Field Entropy threat model    |
| Obf-Key-3                  | Rotation of the obfuscation key (if not driven through PUF) between silicon steppings of a given product (for example, A0 vs. B0 vs. PRQ stepping) is dependent on company-specific policies.                                                                                  | Required by UDS and Field Entropy threat model    |
| Obf-Key-4                  | SoC backend flows should not insert obfuscation key flops into the scan chain.                                                                                                                                                                                                 | Required by UDS and Field Entropy threat model    |
| Obf-Key-5                  | For defense in depth, it is strongly recommended that deobfuscation key flops are not on the scan chain. <br> Remove the following signals from the scan chain: <br> cptra\_scan\_mode\_Latched\_d <br> cptra\_scan\_mode\_Latched\_f <br> field\_storage.internal\_obf\_key   | Caliptra HW threat model                          |
| Obf-Key-6                  | SoC shall ensure that obfuscation key is available (and wires are stable) before Caliptra reset is de-asserted.                                                                                                                                                                | Functionality and security                        |
...
| Sec-State-1                | SoC shall drive security state wires in accordance with the SoC's security state.                                                                                                                                                                                                                                                                                                          | Required for Caliptra threat model                |
| Sec-State-2                | If SoC is under debug, then SoC shall drive debug security state to Caliptra.                                                                                                                                                                                                                                                                                                              | Required for Caliptra threat model                |
| Rst-and-clk-1              | SoC shall start input clock before cptra\_pwrgood assertion. The clock must operate for a minimum of 10 clock cycles before SoC asserts cptra\_pwrgood.                                                                                                                                                                                                                                    | Functional                                        |
| Rst-and-clk-2              | After asserting cptra\_pwrgood, SoC shall wait for a minimum of 10 clock cycles before deasserting cptra\_rst\_b.                                                                                                                                                                                                                                                                          | Functional                                        |
| Rst-and-clk-3              | SoC reset logic shall assume reset assertions are asynchronous and deassertions are synchronous.                                                                                                                                                                                                                                                                                           | Functional                                        |
| Rst-and-clk-4              | SoC shall ensure Caliptra's powergood is tied to SoC’s own powergood or any other reset that triggers SoC’s cold boot flow.                                                                                                                                                                                                                                                                | Required for Caliptra threat model                |
| Rst-and-clk-5              | SoC shall ensure Caliptra clock is derived from an on-die oscillator circuit.                                                                                                                                                                                                                                                                                                              | Required for Caliptra threat model                |
| Rst-and-clk-6              | SoC shall ensure that any programmable Caliptra clock controls are restricted to the SoC Manager.                                                                                                                                                                                                                                                                                          | Required for Caliptra threat model                |
| Rst-and-clk-7              | SoC should defend against external clock stop attacks.                                                                                                                                                                                                                                                                                                                                     | Required for Caliptra threat model                |
| Rst-and-clk-8              | SoC should defend against external clock glitching attacks.                                                                                                                                                                                                                                                                                                                                | Required for Caliptra threat model                |
| Rst-and-clk-9              | SoC should defend against external clock overclocking attacks.                                                                                                                                                                                                                                                                                                                             | Required for Caliptra threat model                |
...
| Backend-convergence-1      | Caliptra supports frequencies up to 400MHz using an industry standard, moderately advanced technology node as of 2023 September. Integrator designs must also achieve backend convergence.                                                                                                                                                                                                                                                           | Functional                                        |
...
| Error-rpt-1                | SoC shall report Caliptra error outputs.                                                                                                                                                                                                                                                                                                                                                   | Telemetry and monitoring                          
| Error-rpt-2                | SoC shall only recover Caliptra fatal errors via SoC power-good reset.                                                                                                                                                                                                                                                                                                                     | Required for Caliptra threat model                |
| TRNG-AXI-USER-1            | If SoC doesn’t program the CPTRA\_TRNG\_AXI\_USER\_LOCK\[LOCK\] and Caliptra is configured in external TRNG mode, then Caliptra HW will accept TRNG data from any SoC entity.                                                                                                                                                                                                            | Required for Caliptra threat model                |
| TRNG-AXI-USER-2            | If SoC programs CPTRA\_TRNG\_VALID\_AXI\_USER and sets CPTRA\_TRNG\_AXI\_USER\_LOCK\[LOCK\] and Caliptra is configured in external TRNG mode, then Caliptra HW will accept TRNG data only from the entity that is programmed into the AXI USER register.                                                                                                                                 | Required for Caliptra threat model                |
| TRNG-AXI-USER-3            | It is strongly recommended that these AXI USER registers are either set at integration time through integration parameters or be programmed by the SoC ROM before any mutable FW or ROM patches are absorbed.                                                                                                                                                                            | Required for Caliptra threat model                |
| TRNG-AXI-USER-4            | It is strongly recommended that integrators set the TRNG valid AXI_USER to a non-zero value, due to the above uniqueness requirement.                                                                                                                                                                                                                                                    | Required for Caliptra threat model                |
| MAILBOX-AXI-USER-1         | 5 AXI USER attribute registers are implemented at SoC interface.                                                                                                                                                                                                                                                                                                                         | Required for Caliptra threat model                |
| MAILBOX-AXI-USER-2         | At boot time, a default SoC or AXI USER can access the mailbox. The value of this AXI USER is an integration parameter, CPTRA\_DEF\_MBOX\_VALID\_AXI\_USER.                                                                                                                                                                                                                              | Required for Caliptra threat model                |
| MAILBOX-AXI-USER-3         | The value of CPTRA\_MBOX\_VALID\_AXI\_USER\[4:0\] register can be programmed by SoC. After it is locked, it becomes a valid AXI USER for accessing the mailbox.                                                                                                                                                                                                                          | Required for Caliptra threat model                |
...
| MAILBOX-AXI-USER-5         | SoC logic (ROM, HW) that is using the Caliptra mailbox right out of cold reset, without first configuring the programmable mailbox AXI USER registers, must send the mailbox accesses with the default AXI USER, CPTRA\_DEF\_MBOX\_VALID\_AXI\_USER.                                                                                                                                     | Required for Caliptra threat model                |
| MAILBOX-AXI-USER-6         | For CPTRA\_MBOX\_VALID\_AXI\_USER\[4:0\], the corresponding lock bits MUST be programmed to ‘1. This enables the mailbox to accept transactions from non-default AXI USERS.                                                                                                                                                                                                              | Required for Caliptra threat model                |
| MAILBOX-AXI-USER-7         | It is strongly recommended that mailbox AXI USER registers are either set at integration time through integration parameters or are programmed by the SoC ROM before any mutable FW or ROM patches are applied.                                                                                                                                                                          | Required for Caliptra threat model                |
| MAILBOX-AXI-USER-8         | It is strongly recommended that integrators set the MAILBOX valid AXI_USER to non-zero values, due to the above uniqueness requirement.                                                                                                                                                                                                                                                  | Required for Caliptra threat model                |
| FUSE-AXI-USER-1            | 1 AXI USER attribute register is implemented at SoC interface: CPTRA\_FUSE\_VALID\_AXI\_USER.                                                                                                                                                                                                                                                                                            | Required for Caliptra threat model                |
| FUSE-AXI-USER-2            | CPTRA\_FUSE\_AXI\_USER\_LOCK locks the programmable valid axi user register, and marks the programmed value as valid.                                                                                                                                                                                                                                                                    | Required for Caliptra threat model                |
...
| FUSE-AXI-USER-4            | It is strongly recommended that integrators set the FUSE valid AXI_USER to a non-zero value, due to the above uniqueness requirement.                                                                                                                                                                                                                                                    | Required for Caliptra threat model                |
| Mfg-1                      | SoC shall provision an IDevID certificate with fields that conform to the requirements described in [Provisioning IDevID during manufacturing](https://github.com/chipsalliance/Caliptra/blob/main/doc/Caliptra.md#provisioning-idevid-during-manufacturing).                                                                                                                              | Functionality                                     |

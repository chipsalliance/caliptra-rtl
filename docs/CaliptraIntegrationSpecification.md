...
<p style="text-align: center;">Version 2.0.4</p>
...
| 3          | strap_ss_strap_generic_3           | Provides the Caliptra ROM with fatal-error reporting control. Bit [0]: RESERVED/unused, must be tied to 0. Bit [1]: Wait for device reset before fatal error reporting. When set to 1 in subsystem mode, ROM waits for the recovery interface `DEVICE_RESET.RESET_CTRL` field to be set to `0x1` (`Reset Device`) before updating `CPTRA_FW_ERROR_FATAL` in the fatal error handler. When clear, ROM reports fatal errors immediately. Bits [31:2]: RESERVED/unused, must be tied to 0. Refer to the Caliptra ROM Specification for additional details. |

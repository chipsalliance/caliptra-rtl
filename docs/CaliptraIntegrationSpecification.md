...
| :--------- | :--------- | :--------- |
| Obfuscation Key (1)                 | SoC backend flows shall generate obfuscation key with appropriate NIST compliance as dictated in the Caliptra RoT specification.                                                                                                                                               | Required by UDS and Field Entropy threat model    |
| Obfuscation Key (2)                 | If not driven through PUF, SoC backend flows shall ECO the obfuscation key before tapeout.                                                                                                                                                                                     | Required by UDS and Field Entropy threat model    |
| Obfuscation Key (3)                 | Rotation of the obfuscation key (if not driven through PUF) between silicon steppings of a given product (for example, A0 vs. B0 vs. PRQ stepping) is dependent on company-specific policies.                                                                                  | Required by UDS and Field Entropy threat model    |
| Obfuscation Key (4)                 | SoC backend flows should not insert obfuscation key flops into the scan chain.                                                                                                                                                                                                 | Required by UDS and Field Entropy threat model    |
| Obfuscation Key (5)                 | For defense in depth, it is strongly recommended that debofuscation key flops are not on the scan chain. <br> Remove the following signals from the scan chain: <br> cptra\_scan\_mode\_Latched\_d <br> cptra\_scan\_mode\_Latched\_f <br> field\_storage.internal\_obf\_key   | Caliptra HW threat model                          |
| Obfuscation Key (6)                 | SoC shall ensure that obfuscation key is available (and wires are stable) before Caliptra reset is de-asserted.                                                                                                                                                                | Functionality and security                        |
| Obfuscation Key (7)                 | SoC shall implement protections for obfuscation key generation logic and protect against debug/sw/scandump visibility.<br>1. Any flops outside of Caliptra that store obfuscation key or parts of the key should be excluded from scandump.<br>2. SoC shall ensure that the obfuscation key is sent only to Caliptra through HW wires, and it is not visible anywhere outside of Caliptra. | Required for Caliptra threat model |
| CSR HMAC Key (1) | SoC backend flows shall generate CSR signing key with appropriate NIST compliance as dictated in the Caliptra ROT specification.                                                                                                                                                                                                                                                                               | Required by Caliptra threat model |
| CSR HMAC Key (2) | SoC backend flows shall ECO the CSR signing key before tapeout.                                                                                                                                                                                                                                                                                                                                                | Required by Caliptra threat model |
| CSR HMAC Key (3) | SoC backend flows should rotate CSR signing key for each project.                                                                                                                                                                                                                                                                                                                                              | Required by Caliptra threat model |
| CSR HMAC Key (4) | SoC backend flows should not insert CSR signing key flops into the scan chain.                                                                                                                                                                                                                                                                                                                                 | Required by Caliptra threat model |
...
| DFT (2)                              | Caliptra’s TAP should be a TAP endpoint.                                                                                                                                                                                                                                                                                                                                                   | Functional requirement                            |
| Mailbox (1)                          | SoC shall provide an access path between the mailbox and the application CPU complex on SoCs with such complexes (for example, Host CPUs and Smart NICs). See the [Sender Protocol](#sender-protocol) section for details about error conditions.                                                                                                                                          | Required for Project Kirkland and TDISP TSM       |
| Fuses (1)                            | SoC shall burn non-field fuses during manufacturing. Required vs. optional fuses are listed in the architectural specification.                                                                                                                                                                                                                                                            | Required for UDS threat model                     |
| Fuses (2)                            | SoC shall expose an interface for burning field fuses. Protection of this interface is the SoC vendor’s responsibility.                                                                                                                                                                                                                                                                    | Required for Field Entropy                        |
| Fuses (3)                            | SoC shall write fuse registers and fuse done via immutable logic or ROM code.                                                                                                                                                                                                                                                                                                              | Required for Caliptra threat model                |
| Fuses (4)                            | SoC shall expose an API for programming Field Entropy as described in the architecture documentation. SoC shall ensure that Field Entropy can only be programmed via this API and shall explicitly prohibit burning of discrete Field Entropy bits and re-burning of already burned Field Entropy entries.                                                                                 | Required for Field Entropy                        |
| Fuses (5)                            | SoC shall ensure that any debug read paths for fuses are disabled in PRODUCTION lifecycle state.                                                                                                                                                                                                                                                                                           | Required for Field Entropy                        |
| Fuses (6)                            | SoC shall ensure that UDS\_SEED and Field Entropy supplied to Caliptra come directly from OTP fuses and there are no debug paths to inject new values.                                                                                                                                                                                                                                     | Required for Caliptra threat model                |
| Fuses (7)                            | SoC shall add integrity checks for Caliptra fuses as per SoC policy.                                                                                                                                                                                                                                                                                                                       | Reliability                                       |
| Fuses (8)                            | SoC should apply shielding/obfuscation measures to protect fuse macro.                                                                                                                                                                                                                                                                                                                     | Required for Caliptra threat model                |
| Fuses (9)                            | SoCs that intend to undergo FIPS 140-3 zeroization shall expose zeroization API as described in zeroization requirements in architecture specification. SoC shall apply appropriate authentication for this API to protect against denial of service and side channel attacks.                                                                                                             | FIPS 140-3 certification                          |
| Security State (1)                   | SoC shall drive security state wires in accordance with the SoC's security state.                                                                                                                                                                                                                                                                                                          | Required for Caliptra threat model                |
| Security State (2)                   | If SoC is under debug, then SoC shall drive debug security state to Caliptra.                                                                                                                                                                                                                                                                                                              | Required for Caliptra threat model                |
| Resets and Clocks (1)                | SoC shall start input clock before cptra\_pwrgood assertion. The clock must operate for a minimum of 10 clock cycles before SoC asserts cptra\_pwrgood.                                                                                                                                                                                                                                    | Functional                                        |
| Resets and Clocks (2)                | After asserting cptra\_pwrgood, SoC shall wait for a minimum of 10 clock cycles before deasserting cptra\_rst\_b.                                                                                                                                                                                                                                                                          | Functional                                        |
| Resets and Clocks (3)                | SoC reset logic shall assume reset assertions are asynchronous and deassertions are synchronous.                                                                                                                                                                                                                                                                                           | Functional                                        |
| Resets and Clocks (4)                | SoC shall ensure Caliptra's powergood is tied to SoC’s own powergood or any other reset that triggers SoC’s cold boot flow.                                                                                                                                                                                                                                                                | Required for Caliptra threat model                |
| Resets and Clocks (5)                | SoC shall ensure Caliptra clock is derived from an on-die oscillator circuit.                                                                                                                                                                                                                                                                                                              | Required for Caliptra threat model                |
| Resets and Clocks (6)                | SoC shall ensure that any programmable Caliptra clock controls are restricted to the SoC Manager.                                                                                                                                                                                                                                                                                          | Required for Caliptra threat model                |
| Resets and Clocks (7)                | SoC should defend against external clock stop attacks.                                                                                                                                                                                                                                                                                                                                     | Required for Caliptra threat model                |
| Resets and Clocks (8)                | SoC should defend against external clock glitching attacks.                                                                                                                                                                                                                                                                                                                                | Required for Caliptra threat model                |
| Resets and Clocks (9)                | SoC should defend against external clock overclocking attacks.                                                                                                                                                                                                                                                                                                                             | Required for Caliptra threat model                |
| TRNG (1)                             | SoC shall either provision Caliptra with a dedicated TRNG or shared TRNG. It is highly recommended to use dedicated ITRNG                                                                                                                                                                                                                                                                  | Required for Caliptra threat model and Functional |
| TRNG (2)                             | SoC shall provision the Caliptra embedded TRNG with an entropy source if that is used (vs. SoC-shared TRNG API support).                                                                                                                                                                                                                                                                   | Functional                                        |
| TRNG (3)                             | If the TRNG is shared, then upon TRNG\_REQ, SoC shall use immutable logic or code to program Caliptra's TRNG registers.                                                                                                                                                                                                                                                                    | Required for Caliptra threat model and Functional |
| SRAMs (1)                            | SoC shall ensure timing convergence with 1-cycle read path for SRAMs.                                                                                                                                                                                                                                                                                                                      | Functional                                        |
| SRAMs (2)                            | SoC shall size SRAMs to account for SECDED. Exception for Adams-Bridge SRAMs that do not utilize SECDED.                                                                                                                                                                                                                                                                                   | Functional |
| SRAMs (3)                            | SoC shall write-protect fuses that characterize the SRAM.                                                                                                                                                                                                                                                                                                                                  | Required for Caliptra threat model |
| SRAMs (4)                            | SoC shall ensure SRAM content is only destroyed on powergood cycling.                                                                                                                                                                                                                                                                                                                      | Functional (Warm Reset, Hitless Update) |
| SRAMs (5)                            | SoC shall only perform SRAM repair on powergood events and prior to caliptra\_rst\_b deassertion. SoC shall also ensure that SRAMs are initialized with all 0 data during powergood events, and prior to caliptra\_rst\_b deassertion.                                                                                                                                                     | Functional (Warm Reset, Hitless Update) |
| Backend convergence (1)              | Caliptra supports frequencies up to 400MHz using an industry standard, moderately advanced technology node as of 2023 September.                                                                                                                                                                                                                                                           | Functional                                        |
| Power saving (1)                     | Caliptra clock gating shall be controlled by Caliptra firmware alone. SoC is provided a global clock gating enable signal (and a register) to control.                                                                                                                                                                                                                                     | Required for Caliptra threat model                |
| Power saving (2)                     | SoC shall not power-gate Caliptra independently of the entire SoC.                                                                                                                                                                                                                                                                                                                         | Required for Caliptra threat model                |
| AXI USER (1)                         | SoC shall drive AXI USER input for all AXI requests that have a route to Caliptra (e.g. through the AXI interconnect). CANNOT use value 0xFFFFFFFF. This is reserved for Caliptra-internal usage.                                                                                                                                                                                          | Required for Caliptra threat model                |
| AXI USER (2)                         | Assigned AXI USER values must be unique for each agent on the interconnect. All SoC AXI agents that have an access path to Caliptra AXI subordinate (or any Caliptra Subsystem components, when using the Subsystem mode) via AXI must either (a) generate AXI transactions using an AXI_USER value that is unique from that used by all other AXI agents on the interconnect or (b) generate AXI transactions using an AXI_USER value that will never overlap with the configured valid AXI users, if the agent is not a valid mailbox user, valid fuse user, or valid trng user. For example, if any AXI agents utilize the AxUSER field for any type of dynamic metadata and are on the same AXI interconnect as Caliptra, all possible AxUSER values from those agents should be avoided for assigning Caliptra VALID AXI USER values. Because AXI AxUSER signals are used to identify accessing agents and enforce access rules, this rule ensures that no single agent may ever generate an AXI transaction that identifies it as originating from a different agent.  | Required for Caliptra threat model                |
| Error reporting (1)                  | SoC shall report Caliptra error outputs.                                                                                                                                                                                                                                                                                                                                                   | Telemetry and monitoring                          
| Error reporting (2)                  | SoC shall only recover Caliptra fatal errors via SoC power-good reset.                                                                                                                                                                                                                                                                                                                     | Required for Caliptra threat model                |
| TRNG AXI USER Programming rules (1)    | If SoC doesn’t program the CPTRA\_TRNG\_AXI\_USER\_LOCK\[LOCK\] and Caliptra is configured in external TRNG mode, then Caliptra HW will accept TRNG data from any SoC entity.                                                                                                                                                                                                            | Required for Caliptra threat model                |
| TRNG AXI USER Programming rules (2)    | If SoC programs CPTRA\_TRNG\_VALID\_AXI\_USER and sets CPTRA\_TRNG\_AXI\_USER\_LOCK\[LOCK\] and Caliptra is configured in external TRNG mode, then Caliptra HW will accept TRNG data only from the entity that is programmed into the AXI USER register.                                                                                                                                 | Required for Caliptra threat model                |
| TRNG AXI USER Programming rules (3)    | It is strongly recommended that these AXI USER registers are either set at integration time through integration parameters or be programmed by the SoC ROM before any mutable FW or ROM patches are absorbed.                                                                                                                                                                            | Required for Caliptra threat model                |
| TRNG AXI USER Programming rules (4)    | It is strongly recommended that integrators set the TRNG valid AXI_USER to a non-zero value, due to the above uniqueness requirement.                                                                                                                                                                                                                                                    | Required for Caliptra threat model                |
| MAILBOX AXI USER programming rules (1) | 5 AXI USER attribute registers are implemented at SoC interface.                                                                                                                                                                                                                                                                                                                         | Required for Caliptra threat model                |
| MAILBOX AXI USER programming rules (2) | At boot time, a default SoC or AXI USER can access the mailbox. The value of this AXI USER is an integration parameter, CPTRA\_DEF\_MBOX\_VALID\_AXI\_USER.                                                                                                                                                                                                                              | Required for Caliptra threat model                |
| MAILBOX AXI USER programming rules (3) | The value of CPTRA\_MBOX\_VALID\_AXI\_USER\[4:0\] register can be programmed by SoC. After it is locked, it becomes a valid AXI USER for accessing the mailbox.                                                                                                                                                                                                                          | Required for Caliptra threat model                |
| MAILBOX AXI USER programming rules (4) | CPTRA\_SET\_MBOX\_AXI\_USER\_INTEG parameter can be set along with the corresponding CPTRA\_MBOX\_VALID\_AXI\_USER parameter at integration time. If set, these integration parameters take precedence over the CPTRA\_MBOX\_VALID\_AXI\_USER\[4:0\] register.                                                                                                                           | Required for Caliptra threat model                |
| MAILBOX AXI USER programming rules (5) | SoC logic (ROM, HW) that is using the Caliptra mailbox right out of cold reset, without first configuring the programmable mailbox AXI USER registers, must send the mailbox accesses with the default AXI USER, CPTRA\_DEF\_MBOX\_VALID\_AXI\_USER.                                                                                                                                     | Required for Caliptra threat model                |
| MAILBOX AXI USER programming rules (6) | For CPTRA\_MBOX\_VALID\_AXI\_USER\[4:0\], the corresponding lock bits MUST be programmed to ‘1. This enables the mailbox to accept transactions from non-default AXI USERS.                                                                                                                                                                                                              | Required for Caliptra threat model                |
| MAILBOX AXI USER programming rules (7) | It is strongly recommended that mailbox AXI USER registers are either set at integration time through integration parameters or are programmed by the SoC ROM before any mutable FW or ROM patches are applied.                                                                                                                                                                          | Required for Caliptra threat model                |
| MAILBOX AXI USER Programming rules (8) | It is strongly recommended that integrators set the MAILBOX valid AXI_USER to non-zero values, due to the above uniqueness requirement.                                                                                                                                                                                                                                                  | Required for Caliptra threat model                |
| FUSE AXI USER programming rules (1)    | 1 AXI USER attribute register is implemented at SoC interface: CPTRA\_FUSE\_VALID\_AXI\_USER.                                                                                                                                                                                                                                                                                            | Required for Caliptra threat model                |
| FUSE AXI USER programming rules (2)    | CPTRA\_FUSE\_AXI\_USER\_LOCK locks the programmable valid axi user register, and marks the programmed value as valid.                                                                                                                                                                                                                                                                    | Required for Caliptra threat model                |
| FUSE AXI USER programming rules (3)    | Integrators can choose to harden the valid axi user for fuse access by setting the integration parameter, CPTRA\_FUSE\_VALID\_AXI\_USER, to the desired value in RTL, and by setting CPTRA\_SET\_FUSE\_AXI\_USER\_INTEG to 1. If set, these integration parameters take precedence over the CPTRA\_FUSE\_VALID\_AXI\_USER register.                                                      | Required for Caliptra threat model                |
| FUSE AXI USER Programming rules (4)    | It is strongly recommended that integrators set the FUSE valid AXI_USER to a non-zero value, due to the above uniqueness requirement.                                                                                                                                                                                                                                                    | Required for Caliptra threat model                |
| Manufacturing (1)                      | SoC shall provision an IDevID certificate with fields that conform to the requirements described in [Provisioning IDevID during manufacturing](https://github.com/chipsalliance/Caliptra/blob/main/doc/Caliptra.md#provisioning-idevid-during-manufacturing).                                                                                                                              | Functionality                                     |
| Manufacturing (2)                      | Caliptra relies on obfuscation for confidentiality of UDS\_SEED. It is strongly advised to implement manufacturing policies to protect UDS\_SEED as defense in depth measures. <br>1, Prevent leakage of UDS\_SEED on manufacturing floor.<br>2. Implement policies to prevent cloning (programming same UDS\_SEED into multiple devices).<br>3. Implement policies to prevent signing of spurious IDEVID certs. | Required for Caliptra threat model |
| Chain of trust (1)                     | SoC shall ensure all mutable code and configuration measurements are stashed into Caliptra. A statement of conformance lists what is considered mutable code and configuration vs. what is not. The statement also describes the start of the boot sequence of the SoC and how Caliptra is incorporated into it.                                                                                                 | Required for Caliptra threat model |
| Chain of trust (2)                     | SoC shall limit the mutable code and configuration that persists across the Caliptra powergood reset. A statement of conformance lists what persists and why this persistence is necessary.                                                                                                                                                                                                                      | Required for Caliptra threat model                |
| Implementation (1)                     | SoC shall apply size-only constraints on cells tagged with the "u\_\_size\_only\_\_" string and shall ensure that these are not optimized in synthesis and PNR                                                                                                                                                                                                                                                   | Required for Caliptra threat model                |
| GLS FEV (1)                            | GLS FEV must be run to make sure netlist and RTL match and none of the countermeasures are optimized away. See the following table for example warnings from synthesis runs to resolve through FEV                                                                                                                                                                                                               | Functional requirement                |

...
| :--------- | :--------- | :--------- |
| Dev-Key-1                       | SoC backend flows shall generate obfuscation and CSR HMAC key with appropriate NIST compliance as dictated in the Caliptra RoT specification.                                                                                                                                                                        | Required by Caliptra threat model |
| Dev-Key-2                       | SoC backend flows shall ECO the obfuscation and CSR HMAC signing keys before tapeout.                                                                                                                                                                                                                                | Required by Caliptra threat model |
| Dev-Key-3*                      | SoC backend flows should rotate obfuscation and CSR HMAC signing keys for each project.                                                                                                                                                                                                                              | Required by Caliptra threat model |
| Dev-Key-4                       | The obfuscation and CSR HMAC keys shall not be accessible (readable or modifiable) to firmware or any on-chip non-Caliptra entities, including preventing oracle attacks. The keys shall not be on any scannable path.                                                                                               | Required by Caliptra threat model |
| Dev-Key-5*                      | SoC backend flows should not insert obfuscation and CSR HMAC key flops into the scan chain.                                                                                                                                                                                                                          | Required by Caliptra threat model |
| Dev-Key-6*                      | For defense in depth, it is strongly recommended that obfuscation and CSR HMAC key flops are not on the scan chain. <br> Remove the following signals from the scan chain: <br> cptra\_scan\_mode\_Latched\_d <br> cptra\_scan\_mode\_Latched\_f <br> field\_storage.internal\_obf\_key <br> cptra_csr_hmac_key_reg  | Required by Caliptra threat model |
| Dev-Key-7                       | SoC shall implement protections for obfuscation and CSR HMAC key generation logic and protect against debug/sw/scandump visibility.<br>1. Any flops outside of Caliptra that store keys or parts of the keys should be excluded from scandump.<br>2. SoC shall ensure that the obfuscation key is sent only to Caliptra through HW wires, and it is not visible anywhere outside of Caliptra. | Required by Caliptra threat model |
...
| Obf-Key-2                       | SoC shall ensure that obfuscation key is available (and wires are stable) before Caliptra reset is de-asserted.                                                                                                                                                                                                      | Functional requirement and Required by Caliptra threat model |
...
| TRNG-2                          | If the TRNG is shared, then upon TRNG\_REQ, SoC shall use immutable logic or code to program Caliptra's TRNG registers.                                                                                                                                                                                              | Functional requirement and Required by Caliptra threat model |
| TRNG-3*                         | SoC shall either provision Caliptra with a dedicated TRNG or shared TRNG. It is highly recommended to use dedicated ITRNG. When using Caliptra Subsystem, dedicated internal TRNG is required for Caliptra.                                                                                                          | Functional requirement and Required by Caliptra threat model |

...
| Dev-Keys-2                      | SoC backend flows shall ECO the obfuscation and CSR HMAC signing keys before tapeout.                                                                                                                                                                                                                                | Required by Caliptra threat model |
...
| Dev-Keys-5*                     | SoC backend flows should not insert obfuscation and CSR HMAC key flops into the scan chain.                                                                                                                                                                                                                          | Required by Caliptra threat model |
| Dev-Keys-6*                     | For defense in depth, it is strongly recommended that obfuscation and CSR HMAC key flops are not on the scan chain. <br> Remove the following signals from the scan chain: <br> cptra\_scan\_mode\_Latched\_d <br> cptra\_scan\_mode\_Latched\_f <br> field\_storage.internal\_obf\_key <br> cptra_csr_hmac_key_reg  | Required by Caliptra threat model |
...
| Obf-Key-3                       | If not driven through PUF, SoC backend flows shall ECO the obfuscation key before tapeout.                                                                                                                                                                                                                           | Required by Caliptra threat model    |
| Obf-Key-4                       | Rotation of the obfuscation key (if not driven through PUF) between silicon steppings of a given product (for example, A0 vs. B0 vs. PRQ stepping) is dependent on company-specific policies.                                                                                                                        | Required by Caliptra threat model    |

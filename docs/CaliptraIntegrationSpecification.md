...
| :--------- | :--------- | :--------- |
| Obf-Key-1                  | SoC backend flows shall generate obfuscation key with appropriate NIST compliance as dictated in the Caliptra RoT specification.                                                                                                                                               | Required by Caliptra threat model    |
| Obf-Key-2                  | If not driven through PUF, SoC backend flows shall ECO the obfuscation key before tapeout.                                                                                                                                                                                     | Required by Caliptra threat model    |
...
| Obf-Key-4                  | SoC backend flows should not insert obfuscation key flops into the scan chain.                                                                                                                                                                                                 | Required by Caliptra threat model    |
| Obf-Key-5                  | For defense in depth, it is strongly recommended that deobfuscation key flops are not on the scan chain. <br> Remove the following signals from the scan chain: <br> cptra\_scan\_mode\_Latched\_d <br> cptra\_scan\_mode\_Latched\_f <br> field\_storage.internal\_obf\_key   | Required by Caliptra threat model    |
| Obf-Key-6                  | SoC shall ensure that obfuscation key is available (and wires are stable) before Caliptra reset is de-asserted.                                                                                                                                                                | Functional requirement and Required by Caliptra threat model |
| Obf-Key-7                  | SoC shall implement protections for obfuscation key generation logic and protect against debug/sw/scandump visibility.<br>1. Any flops outside of Caliptra that store obfuscation key or parts of the key should be excluded from scandump.<br>2. SoC shall ensure that the obfuscation key is sent only to Caliptra through HW wires, and it is not visible anywhere outside of Caliptra. | Required by Caliptra threat model |
...
| Mbox-1                     | SoC shall provide an access path between the mailbox and the application CPU complex on SoCs with such complexes (for example, Host CPUs and Smart NICs). See the [Sender Protocol](#sender-protocol) section for details about error conditions.                                                                                                                                          | Required for Project Kirkland and TDISP TSM       |
| Fuses-1                    | SoC shall burn non-field fuses during manufacturing. Required vs. optional fuses are listed in the architectural specification.                                                                                                                                                                                                                                                            | Required by Caliptra threat model                   |
...
| Fuses-3                    | SoC shall write fuse registers and fuse done via immutable logic or ROM code.                                                                                                                                                                                                                                                                                                              | Required fbyor Caliptra threat model                |
...
| Fuses-8                    | SoC should apply shielding/obfuscation measures to protect fuse macro.                                                                                                                                                                                                                                                                                                                     | Required by Caliptra threat model                |
...
| Rst-and-clk-5              | SoC shall ensure Caliptra clock is derived from an on-die oscillator circuit.                                                                                                                                                                                                                                                                                                              | Required by Caliptra threat model                |
...
| Rst-and-clk-7              | SoC should defend against external clock stop attacks.                                                                                                                                                                                                                                                                                                                                     | Required by Caliptra threat model                |
| Rst-and-clk-8              | SoC should defend against external clock glitching attacks.                                                                                                                                                                                                                                                                                                                                | Required by Caliptra threat model                |
| Rst-and-clk-9              | SoC should defend against external clock overclocking attacks.                                                                                                                                                                                                                                                                                                                             | Required by Caliptra threat model                |
| TRNG-1                     | SoC shall either provision Caliptra with a dedicated TRNG or shared TRNG. It is highly recommended to use dedicated ITRNG                                                                                                                                                                                                                                                                  | Required by Caliptra threat model and Functional Requirement |
| TRNG-2                     | SoC shall provision the Caliptra embedded TRNG with an entropy source if that is used (vs. SoC-shared TRNG API support).                                                                                                                                                                                                                                                                   | Functional Requirement                                       |
| TRNG-3                     | If the TRNG is shared, then upon TRNG\_REQ, SoC shall use immutable logic or code to program Caliptra's TRNG registers.                                                                                                                                                                                                                                                                    | Required by Caliptra threat model and Functional Requirement |
...
| Backend-convergence-1      | Caliptra supports frequencies up to 400MHz using an industry standard, moderately advanced technology node as of 2023 September. Integrator designs must also achieve backend convergence.                                                                                                                                                                                                                                                           | Functional Requirement                                       |
...
| TRNG-AXI-USER-1            | If SoC doesn’t program the CPTRA\_TRNG\_AXI\_USER\_LOCK\[LOCK\] and Caliptra is configured in external TRNG mode, then Caliptra HW will accept TRNG data from any SoC entity.                                                                                                                                                                                                            | Required by Caliptra threat model                |
| TRNG-AXI-USER-2            | If SoC programs CPTRA\_TRNG\_VALID\_AXI\_USER and sets CPTRA\_TRNG\_AXI\_USER\_LOCK\[LOCK\] and Caliptra is configured in external TRNG mode, then Caliptra HW will accept TRNG data only from the entity that is programmed into the AXI USER register.                                                                                                                                 | Required by Caliptra threat model                |
| TRNG-AXI-USER-3            | It is strongly recommended that these AXI USER registers are either set at integration time through integration parameters or be programmed by the SoC ROM before any mutable FW or ROM patches are absorbed.                                                                                                                                                                            | Required by Caliptra threat model                |
| TRNG-AXI-USER-4            | It is strongly recommended that integrators set the TRNG valid AXI_USER to a non-zero value, due to the above uniqueness requirement.                                                                                                                                                                                                                                                    | Required by Caliptra threat model                |
| MAILBOX-AXI-USER-1         | 5 AXI USER attribute registers are implemented at SoC interface.                                                                                                                                                                                                                                                                                                                         | Required by Caliptra threat model                |
| MAILBOX-AXI-USER-2         | At boot time, a default SoC or AXI USER can access the mailbox. The value of this AXI USER is an integration parameter, CPTRA\_DEF\_MBOX\_VALID\_AXI\_USER.                                                                                                                                                                                                                              | Required by Caliptra threat model                |
| MAILBOX-AXI-USER-3         | The value of CPTRA\_MBOX\_VALID\_AXI\_USER\[4:0\] register can be programmed by SoC. After it is locked, it becomes a valid AXI USER for accessing the mailbox.                                                                                                                                                                                                                          | Required by Caliptra threat model                |
| MAILBOX-AXI-USER-4         | CPTRA\_SET\_MBOX\_AXI\_USER\_INTEG parameter can be set along with the corresponding CPTRA\_MBOX\_VALID\_AXI\_USER parameter at integration time. If set, these integration parameters take precedence over the CPTRA\_MBOX\_VALID\_AXI\_USER\[4:0\] register.                                                                                                                           | Required by Caliptra threat model                |
| MAILBOX-AXI-USER-5         | SoC logic (ROM, HW) that is using the Caliptra mailbox right out of cold reset, without first configuring the programmable mailbox AXI USER registers, must send the mailbox accesses with the default AXI USER, CPTRA\_DEF\_MBOX\_VALID\_AXI\_USER.                                                                                                                                     | Required by Caliptra threat model                |
| MAILBOX-AXI-USER-6         | For CPTRA\_MBOX\_VALID\_AXI\_USER\[4:0\], the corresponding lock bits MUST be programmed to ‘1. This enables the mailbox to accept transactions from non-default AXI USERS.                                                                                                                                                                                                              | Required by Caliptra threat model                |
| MAILBOX-AXI-USER-7         | It is strongly recommended that mailbox AXI USER registers are either set at integration time through integration parameters or are programmed by the SoC ROM before any mutable FW or ROM patches are applied.                                                                                                                                                                          | Required by Caliptra threat model                |
| MAILBOX-AXI-USER-8         | It is strongly recommended that integrators set the MAILBOX valid AXI_USER to non-zero values, due to the above uniqueness requirement.                                                                                                                                                                                                                                                  | Required by Caliptra threat model                |
| FUSE-AXI-USER-1            | 1 AXI USER attribute register is implemented at SoC interface: CPTRA\_FUSE\_VALID\_AXI\_USER.                                                                                                                                                                                                                                                                                            | Required by Caliptra threat model                |
| FUSE-AXI-USER-2            | CPTRA\_FUSE\_AXI\_USER\_LOCK locks the programmable valid axi user register, and marks the programmed value as valid.                                                                                                                                                                                                                                                                    | Required by Caliptra threat model                |
| FUSE-AXI-USER-3            | Integrators can choose to harden the valid axi user for fuse access by setting the integration parameter, CPTRA\_FUSE\_VALID\_AXI\_USER, to the desired value in RTL, and by setting CPTRA\_SET\_FUSE\_AXI\_USER\_INTEG to 1. If set, these integration parameters take precedence over the CPTRA\_FUSE\_VALID\_AXI\_USER register.                                                      | Required by Caliptra threat model                |
| FUSE-AXI-USER-4            | It is strongly recommended that integrators set the FUSE valid AXI_USER to a non-zero value, due to the above uniqueness requirement.                                                                                                                                                                                                                                                    | Required by Caliptra threat model                |
...
| Implementation-1           | SoC shall apply size-only constraints on cells tagged with the "u\_\_size\_only\_\_" string and shall ensure that these are not optimized in synthesis and PNR                                                                                                                                                                                                                                                   | Required by Caliptra threat model                |

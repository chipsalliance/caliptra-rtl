...
| cptra_csr_hmac_key | 512 | Input Strap | Asynchronous | CSR HMAC key is driven by SoC at integration time. Ideally this occurs just before tape-in and the knowledge of this key must be protected. The key is latched by Caliptra following deassertion of the noncore warm reset during DEVICE_MANUFACTURING lifecycle state. This key input is ignored in all other lifecycle states.|
...
| security_state | 6 | Input Strap | Synchronous to clk | Security state that Caliptra should take (for example, manufacturing, secure, unsecure, etc.). The strap is latched by Caliptra on cptra_noncore_rst_b deassertion. Any time the state changes to debug mode, all keys, assets, and secrets stored in fuses or key vault are cleared. Cryptography core states are also flushed if they were being used. <br>The strap is encoded as `{debug_locked[3:0], device_lifecycle[1:0]}`: <br>&bull; Bits [5:2] `debug_locked` are a multi-bit (MuBi4) glitch-hardened encoding: `4'h6` (MuBi4True) = debug **locked**, `4'h9` (MuBi4False) = debug **unlocked**. The two legal values are bitwise complements for fault-injection resistance. Any other (invalid) encoding is interpreted as **locked** (fail-secure). Integrators must drive one of the two legal MuBi4 values; a single-bit strap is no longer sufficient. <br>&bull; Bits [1:0] `device_lifecycle`: `2'b00` = DEVICE_UNPROVISIONED, `2'b01` = DEVICE_MANUFACTURING, `2'b11` = DEVICE_PRODUCTION. |
...
| :--------- | :--------- | :--------- |
| Dev-Keys-1                      | SoC backend flows shall generate obfuscation and CSR HMAC key with appropriate NIST compliance as dictated in the Caliptra RoT specification.                                                                                                                                                                        | Required by Caliptra threat model |
| Dev-Keys-2                      | SoC backend flows shall ECO the CSR HMAC signing key before tapeout. If not driven through PUF, SoC backend flows shall ECO the obfuscation key before tapeout.                                                                                                                                                      | Required by Caliptra threat model |
| Dev-Keys-3*                     | SoC backend flows should rotate obfuscation and CSR HMAC signing keys for each project.                                                                                                                                                                                                                              | Required by Caliptra threat model |
| Dev-Keys-4                      | The obfuscation and CSR HMAC keys shall not be accessible (readable or modifiable) to firmware or any on-chip non-Caliptra entities, including preventing oracle attacks. The keys shall not be on any scannable path.                                                                                               | Required by Caliptra threat model |
| Dev-Keys-5*                     | SoC backend flows shall not insert obfuscation and CSR HMAC key flops into the scan chain.                                                                                                                                                                                                                           | Required by Caliptra threat model |
| Dev-Keys-6*                     | For defense in depth, obfuscation and CSR HMAC key flops must not be on the scan chain. <br> Remove the following signals from the scan chain: <br> cptra\_scan\_mode\_Latched\_d <br> cptra\_scan\_mode\_Latched\_f <br> field\_storage.internal\_obf\_key <br> cptra_csr_hmac_key_reg                              | Required by Caliptra threat model |
| Dev-Keys-7                      | SoC shall implement protections for obfuscation and CSR HMAC key generation logic and protect against debug/sw/scandump visibility.<br>1. Any flops outside of Caliptra that store keys or parts of the keys must be excluded from scandump.<br>2. SoC shall ensure that the obfuscation key is sent only to Caliptra through HW wires, and it is not visible anywhere outside of Caliptra. | Required by Caliptra threat model |
| Obf-Key-1                       | The obfuscation key shall be generated using a method that ensures 256 bits of entropy, such as an ESV certified entropy source or an on-die Physically Unclonable Function (PUF) that is compliant with industry standards.                                                                                         | Required by Caliptra threat model |
| Obf-Key-2                       | SoC shall ensure that obfuscation key is available (and wires are stable) before Caliptra reset is de-asserted.                                                                                                                                                                                                      | Required by Caliptra threat model and Functional Requirement |
| Obf-Key-3                       | Rotation of the obfuscation key (if not driven through PUF) between silicon steppings of a given product (for example, A0 vs. B0 vs. PRQ stepping) is dependent on company-specific policies.                                                                                                                        | Required by Caliptra threat model |
| DFT-1                           | Before scan is enabled (separate signal that SoC implements on scan insertion), SoC shall set Caliptra's scan\_mode indication to '1 for 5,000 clocks to allow secrets/assets to be flushed.                                                                                                                         | Required by Caliptra threat model                |
| DFT-2*                          | If connected, Caliptra’s TAP should be a TAP endpoint.                                                                                                                                                                                                                                                               | Functional Requirement                           |
| DFD-1                           | Integrators shall not connect Caliptra's exposed RISC-V trace ports to any SoC logic. These ports are unvalidated and any implications to the SoC logic due to connecting these signals have not been analyzed.                                                                                                      | Required by Caliptra threat model                |
| Mbox-1                          | SoC should provide an access path between the mailbox and the application CPU complex on SoCs with such complexes (for example, Host CPUs and Smart NICs).                                                                                                                                                           | Required for Project Kirkland and TDISP TSM      |
| Fuses-1                         | SoC shall burn non-field fuses during manufacturing. All *Caliptra Core* fuses must be implemented as described in the [fuse map table](https://github.com/chipsalliance/Caliptra/blob/main/doc/Caliptra.md#fuse-map). Fuses used for *Subsystem* are optional outside of the subsystem context.                     | Required by Caliptra threat model                |
| Fuses-2                         | SoC shall expose an interface for burning field fuses. Protection of this interface is the SoC vendor’s responsibility.                                                                                                                                                                                              | Required by Caliptra threat model                |
| Fuses-3                         | SoC shall write fuse registers and fuse done via immutable logic or ROM code.                                                                                                                                                                                                                                        | Required by Caliptra threat model                |
| Fuses-4                         | SoC shall expose an API for programming Field Entropy as described in the architecture documentation. SoC shall ensure that Field Entropy can only be programmed via this API and shall explicitly prohibit burning of discrete Field Entropy bits and re-burning of already burned Field Entropy entries.           | Required by Caliptra threat model                |
| Fuses-5                         | SoC shall ensure that any debug read paths for fuses are disabled in PRODUCTION lifecycle state.                                                                                                                                                                                                                     | Required by Caliptra threat model                |
| Fuses-6                         | SoC shall ensure that UDS\_SEED and Field Entropy supplied to Caliptra come directly from OTP fuses and there are no debug paths to inject new values.                                                                                                                                                               | Required by Caliptra threat model                |
| Fuses-7                         | SoC shall add integrity checks for Caliptra fuses as per SoC policy.                                                                                                                                                                                                                                                 | Functional Requirement                           |
| Fuses-8                         | SoCs that intend to undergo FIPS 140-3 zeroization shall expose zeroization API as described in zeroization requirements in architecture specification. SoC shall apply appropriate authentication for this API to protect against denial of service.                                       | FIPS 140-3 certification                         |
| Sec-State-1                     | SoC shall drive security state wires in accordance with the SoC's security state.                                                                                                                                                                                                                                    | Required by Caliptra threat model                |
| Sec-State-2                     | If SoC is under debug, then SoC shall drive debug security state to Caliptra.                                                                                                                                                                                                                                        | Required by Caliptra threat model                |
| Rst-and-clk-1                   | SoC shall start input clock before cptra\_pwrgood assertion. The clock must operate for a minimum of 10 clock cycles before SoC asserts cptra\_pwrgood.                                                                                                                                                              | Functional Requirement                           |
| Rst-and-clk-2                   | After asserting cptra\_pwrgood, SoC shall wait for a minimum of 10 clock cycles before deasserting cptra\_rst\_b.                                                                                                                                                                                                    | Functional Requirement                           |
| Rst-and-clk-3                   | SoC reset logic shall assume reset assertions are asynchronous and deassertions are synchronous.                                                                                                                                                                                                                     | Functional Requirement                           |
| Rst-and-clk-4                   | SoC shall ensure Caliptra's powergood is tied to SoC’s own powergood or any other reset that triggers SoC’s cold boot flow.                                                                                                                                                                                          | Required by Caliptra threat model                |
| Rst-and-clk-5                   | SoC shall ensure Caliptra's clock is driven by an on-die source such as an internal oscillator, PLL, or other clock generation circuit. This is to protect against clock fault injection or clock stretching attacks.                                                                                                | Required by Caliptra threat model                |
| Rst-and-clk-6                   | SoC shall ensure that any programmable Caliptra clock controls are restricted to the SoC Manager.                                                                                                                                                                                                                    | Required by Caliptra threat model                |
| Rst-and-clk-7                   | SoC should employ reasonable countermeasures to defend against advanced physical attacks like clock glitching. These countermeasures may be guided by common best practices as well as specific customer requirements.                                                                                               | Required by Caliptra threat model                |
| TRNG-1                          | SoC shall provision the Caliptra embedded TRNG with an entropy source if that is used (vs. SoC-shared TRNG API support).                                                                                                                                                                                             | Functional Requirement                           |
| TRNG-2                          | If the TRNG is shared, then upon TRNG\_REQ, SoC shall use immutable logic or code to program Caliptra's TRNG registers.                                                                                                                                                                                              | Required by Caliptra threat model and Functional Requirement |
| TRNG-3*                         | SoC shall either provision Caliptra with a dedicated TRNG or shared TRNG. It is highly recommended to use dedicated ITRNG. When using Caliptra Subsystem, dedicated internal TRNG is required for Caliptra.                                                                                                          | Required by Caliptra threat model and Functional Requirement |
| SRAMs-1                         | SoC shall ensure timing convergence with 1-cycle read path for SRAMs.                                                                                                                                                                                                                                                | Functional Requirement                           |
| SRAMs-2                         | SoC shall size SRAMs to account for SECDED. Exception for Adams-Bridge SRAMs that do not utilize SECDED.                                                                                                                                                                                                             | Functional Requirement                           |
| SRAMs-3                         | SoC shall write-protect fuses that characterize the SRAM.                                                                                                                                                                                                                                                            | Required by Caliptra threat model |
| SRAMs-4                         | SoC shall ensure SRAM content is only destroyed on powergood cycling to accommodate warm resets and hitless updates.                                                                                                                                                                                                 | Functional Requirement                           |
| SRAMs-5                         | SoC shall only perform SRAM repair on powergood events and prior to caliptra\_rst\_b deassertion. SoC shall also ensure that SRAMs are initialized with all 0 data during powergood events, and prior to caliptra\_rst\_b deassertion. Necessary for warm resets and hitless updates.                                | Functional Requirement                           |
| Backend-convergence-1           | Caliptra supports frequencies up to 400MHz 0.72V using an industry standard, advanced technology node as of 2025 April. Integrator designs must also achieve backend convergence.                                                                                                                                    | Functional Requirement                           |
| Power-saving-1                  | Caliptra clock gating shall be controlled by Caliptra firmware alone. SoC is provided a global clock gating enable signal (and a register) to control.                                                                                                                                                               | Required by Caliptra threat model                |
| Power-saving-2                  | SoC shall not power-gate Caliptra independently of the entire SoC.                                                                                                                                                                                                                                                   | Required by Caliptra threat model                |
| AXI-USER-1                      | SoC shall drive AXI USER input for all AXI requests that have a route to Caliptra (e.g. through the AXI interconnect). CANNOT use value 0xFFFFFFFF. This is reserved for Caliptra-internal usage.                                                                                                                    | Required by Caliptra threat model                |
| AXI-USER-2                      | Assigned AXI USER values must be unique for each agent on the interconnect. All SoC AXI agents that have an access path to Caliptra AXI subordinate (or any Caliptra Subsystem components, when using the Subsystem mode) via AXI must either (a) generate AXI transactions using an AXI_USER value that is unique from that used by all other AXI agents on the interconnect or (b) generate AXI transactions using an AXI_USER value that will never overlap with the configured valid AXI users, if the agent is not a valid mailbox user, valid fuse user, or valid trng user. For example, if any AXI agents utilize the AxUSER field for any type of dynamic metadata and are on the same AXI interconnect as Caliptra, all possible AxUSER values from those agents should be avoided for assigning Caliptra VALID AXI USER values. Because AXI AxUSER signals are used to identify accessing agents and enforce access rules, this rule ensures that no single agent may ever generate an AXI transaction that identifies it as originating from a different agent.  | Required by Caliptra threat model                |
| Error-rpt-1                     | SoC shall report Caliptra error outputs.                                                                                                                                                                                                                                                                             | Functional Requirement                           |
| Error-rpt-2                     | SoC shall only recover Caliptra fatal errors via SoC power-good reset.                                                                                                                                                                                                                                               | Required by Caliptra threat model                |
| TRNG-AXI-USER-1                 | If integrating Caliptra in passive mode with an external TRNG, SoC integrators must assign an SoC agent that is permitted to access Caliptra's TRNG REQ HW API via AXI. All other AXI agents must be excluded from such access. SoC integrators must enforce this exclusivity either by using the access restriction features described in [TRNG REQ HW API](#TRNG-REQ-HW-API) or by implementing an alternative mechanism. For example, restricting the hardware paths to Caliptra through an AXI interconnect to only the agent that is allowed access also satisfies this requirement. | Required by Caliptra threat model                |
| TRNG-AXI-USER-2*                | It is strongly recommended that the TRNG AXI USER register be programmed by the SoC ROM before any mutable FW or ROM patches are absorbed.                                                                                                                                                                           | Required by Caliptra threat model                |
| TRNG-AXI-USER-3*                | It is strongly recommended that integrators set the TRNG valid AXI_USER to a non-zero value, due to the AXI USER uniqueness requirement.                                                                                                                                                                             | Required by Caliptra threat model                |
| MBOX-AXI-USER-1                 | SoC logic (ROM, HW) that uses the Caliptra mailbox right out of cold reset, without first configuring the programmable mailbox AXI USER registers, must send the mailbox accesses with the default AXI USER, CPTRA\_DEF\_MBOX\_VALID\_AXI\_USER.                                                                     | Required by Caliptra threat model                |
| MBOX-AXI-USER-2                 | SoC integrators must assign a subset of SoC agents that are permitted to access Caliptra's mailbox via AXI. All other AXI agents must be excluded from such access. SoC integrators must enforce this exclusivity either by using the access restriction features described in [MAILBOX AXI USER attribute register](#MAILBOX-AXI-USER-attribute-register) or by implementing an alternative mechanism. For example, restricting the hardware paths to Caliptra through an AXI interconnect to only those agents that are allowed access also satisfies this requirement.  | Required by Caliptra threat model                |
| MBOX-AXI-USER-3*                | It is strongly recommended that mailbox AXI USER registers are either set at integration time through integration parameters or are programmed by the SoC ROM before any mutable FW or ROM patches are applied.                                                                                                      | Required by Caliptra threat model                |
| MBOX-AXI-USER-4*                | It is strongly recommended that integrators set the MAILBOX valid AXI_USER to non-zero values, due to the AXI USER uniqueness requirement.                                                                                                                                                                           | Required by Caliptra threat model                |
| FUSE-AXI-USER-1                 | SoC integrators must assign a single SoC agent that is permitted to access Caliptra's fuse registers via AXI. All other AXI agents must be excluded from such access. SoC integrators must enforce this exclusivity either by using the access restriction features described in [FUSE AXI USER attribute](#FUSE-AXI-USER-attribute) or by implementing an alternative mechanism. For example, restricting the hardware paths to Caliptra through an AXI interconnect to only the agent that is allowed access to fuse registers also satisfies this requirement.  | Required by Caliptra threat model                |
| FUSE-AXI-USER-2*                | It is strongly recommended that fuse AXI USER register is either set at integration time through the integration parameter or is programmed by the SoC ROM before any mutable FW or ROM patches are applied.                                                                                                         | Required by Caliptra threat model                |
| FUSE-AXI-USER-3*                | It is strongly recommended that integrators set the FUSE valid AXI_USER to a non-zero value, due to the AXI USER uniqueness requirement.                                                                                                                                                                             | Required by Caliptra threat model                |
| Mfg-1                           | SoC shall provision an IDevID certificate with fields that conform to the requirements described in [Provisioning IDevID during manufacturing](https://github.com/chipsalliance/Caliptra/blob/main/doc/Caliptra.md#provisioning-idevid-during-manufacturing).                                                        | Functional Requirement                           |
| Mfg-2                           | Caliptra relies on obfuscation for confidentiality of UDS\_SEED. It is strongly advised to implement manufacturing policies to protect UDS\_SEED as defense in depth measures. <br>1. Prevent leakage of UDS\_SEED on manufacturing floor.<br>2. Implement policies to prevent cloning (programming same UDS\_SEED into multiple devices).<br>3. Implement policies to prevent signing of spurious IDEVID certs. | Required by Caliptra threat model |
| Chain-of-trust-1                | SoC shall ensure all mutable code and configuration measurements are stashed into Caliptra. A statement of conformance lists what is considered mutable code and configuration vs. what is not. The statement also describes the start of the boot sequence of the SoC and how Caliptra is incorporated into it.     | Required by Caliptra threat model                |
| Chain-of-trust-2                | SoC shall limit the mutable code and configuration that persists across the Caliptra powergood reset. A statement of conformance lists what persists and why this persistence is necessary.                                                                                                                          | Required by Caliptra threat model                |
| Implementation-1                | Integrators shall follow the [Primitive Instantiation Process](#primitive-instantiation) to ensure that process-specific library cells are used and appropriately named. Integrators shall apply size-only constraints on the resulting cells tagged with the "u\_\_size\_only\_\_" string. Integrators shall review results from synthesis and PNR to ensure that these are not optimized away.                 | Required by Caliptra threat model                |
| GLS-FEV-1                       | GLS FEV must be run to make sure netlist and RTL match. See the following table for example warnings from synthesis runs to resolve through FEV.                                                                                                                                                                     | Functional Requirement                |

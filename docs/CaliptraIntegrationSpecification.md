...
| :--------- | :--------- | :--------- |
| Device Keys  (1)                 | SoC backend flows shall generate obfuscation and CSR HMAC key with appropriate NIST compliance as dictated in the Caliptra RoT specification.                                                                                                                                                                        | Required by UDS and Field Entropy threat model    |
| Device Keys  (2)                 | SoC backend flows shall ECO the obfuscation and CSR HMAC signing keys before tapeout.                                                                                                                                                                                                                                | Required by Caliptra threat model |
| Device Keys* (3)                 | SoC backend flows should rotate obfuscation and CSR HMAC signing keys for each project.                                                                                                                                                                                                                              | Required by Caliptra threat model |
| Device Keys  (4)                 | The obfuscation and CSR HMAC keys shall not be accessible (readable or modifiable) to firmware or any on-chip non-Caliptra entities, including preventing oracle attacks. The keys shall not be on any scannable path.                                                                                               | Required for Caliptra threat model |
| Device Keys* (5)                 | SoC backend flows should not insert obfuscation and CSR HMAC key flops into the scan chain.                                                                                                                                                                                                                          | Required by UDS and Field Entropy threat model    |
| Device Keys* (6)                 | For defense in depth, it is strongly recommended that obfuscation and CSR HMAC key flops are not on the scan chain. <br> Remove the following signals from the scan chain: <br> cptra\_scan\_mode\_Latched\_d <br> cptra\_scan\_mode\_Latched\_f <br> field\_storage.internal\_obf\_key <br> cptra_csr_hmac_key_reg  | Caliptra HW threat model                          |
| Device Keys  (7)                 | SoC shall implement protections for obfuscation and CSR HMAC key generation logic and protect against debug/sw/scandump visibility.<br>1. Any flops outside of Caliptra that store keys or parts of the keys should be excluded from scandump.<br>2. SoC shall ensure that the obfuscation key is sent only to Caliptra through HW wires, and it is not visible anywhere outside of Caliptra. | Required for Caliptra threat model |
| Obfuscation Key (1)              | The obfuscation key shall be generated using a method that ensures 256 bits of entropy, such as an ESV certified entropy source or an on-die Physically Unclonable Function (PUF) that is compliant with industry standards.                                                                                         | Required for Caliptra threat model |
| Obfuscation Key (2)              | SoC shall ensure that obfuscation key is available (and wires are stable) before Caliptra reset is de-asserted.                                                                                                                                                                                                      | Functionality and security                        |
| Obfuscation Key (3)              | If not driven through PUF, SoC backend flows shall ECO the obfuscation key before tapeout.                                                                                                                                                                                                                           | Required by UDS and Field Entropy threat model    |
| Obfuscation Key (4)              | Rotation of the obfuscation key (if not driven through PUF) between silicon steppings of a given product (for example, A0 vs. B0 vs. PRQ stepping) is dependent on company-specific policies.                                                                                                                        | Required by UDS and Field Entropy threat model    |
...
| DFD  (1)                         | Integrators shall not connect Caliptra's exposed RISC-V trace ports to any SoC logic. These ports are unvalidated and any implications to the SoC logic due to connecting these signals have not been analyzed.                                                                                                      | Required for Caliptra threat model                |
| Mailbox (1)                      | SoC should provide an access path between the mailbox and the application CPU complex on SoCs with such complexes (for example, Host CPUs and Smart NICs).                                                                                                                                                           | Required for Project Kirkland and TDISP TSM       |
| Fuses (1)                        | SoC shall burn non-field fuses during manufacturing. All *Caliptra Core* fuses must be implemented as described in the [fuse map table](https://github.com/chipsalliance/Caliptra/blob/main/doc/Caliptra.md#fuse-map). Fuses used for *Subsystem* are optional outside of the subsystem context.                     | Required for UDS threat model                     |
...
| Resets and Clocks (5)            | SoC shall ensure Caliptra's clock is driven by an on-die source such as an internal oscillator, PLL, or other clock generation circuit. This is to protect against clock fault injection or clock stretching attacks.                                                                                                | Required for Caliptra threat model                |
...
| Resets and Clocks (7)            | SoC should employ reasonable countermeasures to defend against advanced physical attacks like clock glitching. These countermeasures may be guided by common best practices as well as specific customer requirements.                                                                                               | Required for Caliptra threat model                |
| TRNG (1)                         | SoC shall provision the Caliptra embedded TRNG with an entropy source if that is used (vs. SoC-shared TRNG API support).                                                                                                                                                                                             | Functional                                        |
| TRNG (2)                         | If the TRNG is shared, then upon TRNG\_REQ, SoC shall use immutable logic or code to program Caliptra's TRNG registers.                                                                                                                                                                                              | Required for Caliptra threat model and Functional |
| TRNG*(3)                         | SoC shall either provision Caliptra with a dedicated TRNG or shared TRNG. It is highly recommended to use dedicated ITRNG. When using Caliptra Subsystem, dedicated internal TRNG is required for Caliptra.                                                                                                          | Required for Caliptra threat model and Functional |
...
| TRNG AXI USER Programming rules  (1)| If integrating Caliptra in passive mode with an external TRNG, SoC integrators must assign an SoC agent that is permitted to access Caliptra's TRNG REQ HW API via AXI. All other AXI agents must be excluded from such access. SoC integrators must enforce this exclusivity either by using the access restriction features described in [TRNG REQ HW API](#TRNG-REQ-HW-API) or by implementing an alternative mechanism. For example, restricting the hardware paths to Caliptra through an AXI interconnect to only the agent that is allowed access also satisfies this requirement. | Required for Caliptra threat model                |
| TRNG AXI USER Programming rules* (2)| It is strongly recommended that the TRNG AXI USER register be programmed by the SoC ROM before any mutable FW or ROM patches are absorbed.                                                                                                                                                                        | Required for Caliptra threat model                |
| TRNG AXI USER Programming rules* (3)| It is strongly recommended that integrators set the TRNG valid AXI_USER to a non-zero value, due to the AXI USER uniqueness requirement.                                                                                                                                                                          | Required for Caliptra threat model                |
| MAILBOX AXI USER Programming rules  (1)| SoC logic (ROM, HW) that uses the Caliptra mailbox right out of cold reset, without first configuring the programmable mailbox AXI USER registers, must send the mailbox accesses with the default AXI USER, CPTRA\_DEF\_MBOX\_VALID\_AXI\_USER.                                                               | Required for Caliptra threat model                |
| MAILBOX AXI USER Programming rules  (2)| SoC integrators must assign a subset of SoC agents that are permitted to access Caliptra's mailbox via AXI. All other AXI agents must be excluded from such access. SoC integrators must enforce this exclusivity either by using the access restriction features described in [MAILBOX AXI USER attribute register](#MAILBOX-AXI-USER-attribute-register) or by implementing an alternative mechanism. For example, restricting the hardware paths to Caliptra through an AXI interconnect to only those agents that are allowed access also satisfies this requirement.  | Required for Caliptra threat model                |
| MAILBOX AXI USER Programming rules* (3)| It is strongly recommended that mailbox AXI USER registers are either set at integration time through integration parameters or are programmed by the SoC ROM before any mutable FW or ROM patches are applied.                                                                                                | Required for Caliptra threat model                |
| MAILBOX AXI USER Programming rules* (4)| It is strongly recommended that integrators set the MAILBOX valid AXI_USER to non-zero values, due to the AXI USER uniqueness requirement.                                                                                                                                                                     | Required for Caliptra threat model                |
| FUSE AXI USER Programming rules  (1)| SoC integrators must assign a single SoC agent that is permitted to access Caliptra's fuse registers via AXI. All other AXI agents must be excluded from such access. SoC integrators must enforce this exclusivity either by using the access restriction features described in [FUSE AXI USER attribute](#FUSE-AXI-USER-attribute) or by implementing an alternative mechanism. For example, restricting the hardware paths to Caliptra through an AXI interconnect to only the agent that is allowed access to fuse registers also satisfies this requirement.  | Required for Caliptra threat model                |
| FUSE AXI USER Programming rules* (2)| It is strongly recommended that fuse AXI USER register is either set at integration time through the integration parameter or is programmed by the SoC ROM before any mutable FW or ROM patches are applied.                                                                                                      | Required for Caliptra threat model                |
| FUSE AXI USER Programming rules* (3)| It is strongly recommended that integrators set the FUSE valid AXI_USER to a non-zero value, due to the AXI USER uniqueness requirement.                                                                                                                                                                          | Required for Caliptra threat model                |

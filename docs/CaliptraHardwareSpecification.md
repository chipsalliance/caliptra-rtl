...
* Non-deterministic (`ECC_CTRL.RAND_K_EN = 1`, FIPS 186-4):
    ...
    `sign_nonce` is a fresh 384-bit value produced by a dedicated `SIGN_NONCE_ST` HMAC_DRBG run issued just before the k-generation. Its entropy is the SCA entropy register (`ECC_IV ^ LFSR_seed ^ counter`) and its nonce is the internal counter; the result is captured into `sign_nonce_reg` and consumed as the nonce input to the k-generation DRBG. The API-visible `ECC_NONCE` register is **not** consumed in SIGN; it is used only by KeyGen.

...
* Deterministic (`ECC_CTRL.RAND_K_EN = 0`, RFC 6979):

        K = HMAC_DRBG(privkey, hashed_msg)

* Non-deterministic (`ECC_CTRL.RAND_K_EN = 1`, FIPS 186-4):

        K = HMAC_DRBG(ECC_SEED, sign_nonce)

    `sign_nonce` is a fresh DRBG output produced by a dedicated `SIGN_NONCE_ST` HMAC_DRBG run issued just before the k-generation. Its width matches the active curve (384-bit for P-384, 256-bit for P-256, zero-extended into the engine's 384-bit datapath). Its entropy is the SCA entropy register (`ECC_IV ^ LFSR_seed ^ counter`) and its nonce is the internal counter; the result is captured into `sign_nonce_reg` and consumed as the nonce input to the k-generation DRBG. The API-visible `ECC_NONCE` register is **not** consumed in SIGN; it is used only by KeyGen.
...
* `RAND_K_EN = 1`: non-deterministic SIGN per FIPS 186-4. `k = HMAC_DRBG(ECC_SEED, sign_nonce)`, where `sign_nonce` is a fresh DRBG output produced internally by an additional `SIGN_NONCE_ST` HMAC_DRBG run (width matches the active curve: 384-bit for P-384, 256-bit for P-256; entropy = `ECC_IV ^ LFSR_seed ^ counter`, nonce = internal counter) issued immediately before the k-generation DRBG. The API-visible `ECC_NONCE` register is not consumed in SIGN. Firmware must write a fresh random value to `ECC_SEED` (and refresh `ECC_IV`, which feeds all SCA countermeasures) before each SIGN command.
...
* Generate a random number k in the range [1..n-1]: k = HMAC\_DRBG(privKey, h) when `ECC_CTRL.RAND_K_EN = 0`; or k = HMAC\_DRBG(ECC\_SEED, sign\_nonce) when `ECC_CTRL.RAND_K_EN = 1`, where `sign\_nonce` is a curve-dependent value (384-bit for P-384, 256-bit for P-256) produced internally by an extra `SIGN_NONCE_ST` HMAC\_DRBG run (entropy derived from `ECC_IV`, the internal LFSR seed, and a counter). `ECC_NONCE` is not consumed in SIGN.
...
| zeroize                    | input           | The core clears all internal registers to avoid any SCA information leakage.                                               |
| seed \[383:0\]             | input           | The deterministic seed for HMAC_DRBG in the KeyGen operation. Also used as the HMAC_DRBG entropy input for the SIGN operation when `ECC_CTRL.RAND_K_EN = 1`. |
| nonce \[383:0\]            | input           | The deterministic nonce for HMAC_DRBG in the KeyGen operation. Not consumed in SIGN: for non-deterministic SIGN (`ECC_CTRL.RAND_K_EN = 1`), the k-generation HMAC_DRBG nonce is an internal `sign_nonce` produced by an extra `SIGN_NONCE_ST` HMAC_DRBG run (entropy from `ECC_IV`, LFSR seed, and counter). |
...
The pseudocode above shows the deterministic SIGN flow (`ECC_CTRL.RAND_K_EN = 0`). When `RAND_K_EN = 1`, the only change is the `k` generation step: replace `k = HMAC_DRBG(privKey, h)` with `k = HMAC_DRBG(ECC_SEED, sign_nonce)`, where `sign_nonce` is a fresh curve-dependent value (384-bit for P-384, 256-bit for P-256) produced by an internal `SIGN_NONCE_ST` HMAC_DRBG run (entropy = `ECC_IV ^ LFSR_seed ^ counter`, nonce = internal counter) issued immediately before the k-generation DRBG; `ECC_NONCE` is not consumed in SIGN. Signing with `RAND_K_EN = 1` is not permitted when the private key is sourced from the PCR vault (PCR_SIGN mode).
